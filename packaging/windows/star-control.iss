...
english.CodexNoticeMessage=Star-Control uses an installed local Marketplace. After setup, run star integration status. If manual_commands are present, complete them in the official Codex CLI or Plugin screen and open a new task. Review and trust the Star-Control Hooks, including SessionStart and SessionEnd, in the Codex CLI /hooks browser; the Desktop settings screen is not the Hook browser. Setup never edits Codex config, cache, or Hook trust files directly.
...
korean.CodexNoticeMessage=Star-Control은 설치된 로컬 Marketplace를 사용합니다. 설치 뒤 star integration status를 실행하세요. manual_commands가 있으면 공식 Codex CLI나 Plugin 화면에서 완료하고 새 작업을 여세요. SessionStart와 SessionEnd를 포함한 Star-Control Hook은 Codex CLI /hooks browser에서 검토하고 신뢰해야 하며 Desktop 설정 화면은 Hook browser가 아닙니다. 설치 파일은 Codex 설정, cache 또는 Hook 신뢰 파일을 직접 수정하지 않습니다.
