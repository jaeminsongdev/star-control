...
- 현재 범위는 P-0078~P-0086이다. source·manifest·Catalog가 정본이고 DB/index/cache는 derived state다.
...
|---|---|---|---|
| P-0078 | Ready 이후 lifecycle-bound retention, lease/cancel/join/checkpoint, hold·migration backup floor, 별도 compaction | source와 설치 Runtime에서 active operation idle grace·crash/restart·hold·bounded batch를 구현했다. 실제 7.12GB store는 official backup set `bks_01KYW284RE97ZEMH5KJ3AZY2HP`를 생성했고 read-only compaction plan은 예상 reclaim 0이라 apply하지 않았다. startup maintenance `opn_01KYWBTX5CP0WP6QC4APK2FNW4`는 terminal complete다. | 최종 Runtime revision에서 cold status와 zero-delete retention plan을 재확인한다. |
| P-0079 | 32/256 item, 65,536-byte hard limit, opaque revision cursor의 management status page | legacy oversize·stale cursor를 fail closed하고 Controller/CLI `--all` 총량 cap을 구현했다. source focused test와 설치 Runtime status paging이 통과했다. | 최종 Runtime에서 status/read budget을 재확인한다. |
| P-0080 | help-first parsing, `command describe --json`, token-aware alias audit, recipe→change→patch lifecycle | canonical/legacy route, descriptor hash, Skill introspection과 updater terminal receipt 검증을 구현했다. CLI 회귀 테스트가 통과했다. | 최종 installer의 source revision·stage/installed hash를 readback한다. |
| P-0081 | typed 8-Hook set, Rules audit, granular approvals | repository validator는 `rules=16`, `hooks=8`을 확인한다. 전역 설정은 backup `C:\Users\thdqu\.codex\backups\star-control-p0081-2aa532a` 뒤 granular approval과 project Rules로 적용했다. Codex `hooks/list` 실측은 8/8 `enabled=true`, `trusted`, warnings/errors 0이다. | 최종 installer가 Hook hash를 바꾸면 동일 live 검토를 다시 수행한다. |
...
| P-0086 | source·Runtime·Registry·Profile·Code Health·Evaluation 최종 봉인과 delivery | 설치 Runtime은 `a85d720445200c784fcdbd571d4e934b88a0ef2d`, Doctor 4/4, Registry revision 16이다. 실제 Finding 4,103건 목록은 64KiB 이하 projection으로 확인했다. 최종 source 후보는 긴 `project.register`, 직접 CLI `scan.run`의 bounded 응답과 정상 Windows 자식 프로세스의 5초 bounded settle을 보강했다. Controller 97, IPC 26+property 1, Clippy와 short/long descendant 회귀가 통과했다. | source evidence/FULL/STRICT → final installer/update → register/full scan/index/Profile/Evaluation shadow → force-free push/readback 순서로 닫는다. |
