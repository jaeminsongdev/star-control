...
- M5 local provider, M6 cloud provider approval gate, M7a CLI control commands, M7b daemon queue skeleton, M7c/M7d API service, M8 UI library model, M9a~M9i observability/security/conformance/recovery/release-readiness foundation은 현재 exit criteria가 코드/fixture로 커버되었고, 현재 구현 축은 release profile integration, release readiness CLI/UI read surface, 또는 명시적 recovery command surface 순서다.
