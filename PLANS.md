...
| P-0038 | DONE — installed Bootstrap v2 and Runtime E2E | 고정 Bootstrap Bridge·Runtime Generation 전환 | schema/Bridge selector, package generation stage verifier, verified install-tree stage, manifest-based candidate review, `star update status\|verify\|stage\|inspect\|apply`, CLI supervisor (authenticated Controller drain → atomic activation → start/postcheck → rollback), Runtime Controller의 Bootstrap+generation IPC peer allowlist를 구현했다. `FULL` PASS: `target/validation/20260718T030810693Z-10288/report.json` (10/10). x64 offline installer가 `D:\도구\Star-Control`에 성공 설치(`installer_exit: 0`)했고, fixed `star-mcp.exe` SHA-256이 stage와 일치한다. installed MCP Registry/describe가 active Runtime Controller를 정상 호출해 기존 `IPC_SERVER_IDENTITY_MISMATCH`를 해소했다. `rt_780b… → rt_9fe…` 전진 cutover, `rt_9fe… → rt_780b…` rollback cutover, 최종 `rt_780b… → rt_9fe…` 재전진이 모두 승인 hash gate·postcheck와 함께 성공했다. 최종 activation revision 4는 active `rt_9fe…`, previous `rt_780b…`를 보존하며, 세 cutover 동안 fixed `star-mcp` PID는 유지되고 Controller generation/PID만 교체됐다. Bridge v1→v2만 fixed Gateway/manifest 교체와 Codex/MCP 재기동이 필요한 1회 migration이며 이후 routine generation update는 그 경계를 통과하지 않는다. |
| P-0039 | DONE — updater/lifecycle | 전용 `star-updater.exe`와 Codex 전체 task 생명주기 자동화 | 4 EXE package·runtime update supervisor 이관, Hook WorkSession counters/30초 lease, exact Desktop process census와 10초 repair-restart, MCP EOF 및 verified Desktop-owner death cleanup·전역 named update mutex를 구현·unit 검증했다. Windows adapter와 installer build script도 `star-updater.exe`를 release-required input으로 강제하고 full release-stage `inspect`는 integration/runtime/updater/mixed 자동 분류와 hash-bound approval scope를 반환한다. updater는 install-root lock을 피하는 staged copy를 local WMI `Win32_Process.Create` broker에서 시작한다. installed-root x64 offline installer E2E(`upd_pYIT…`)는 10초 종료→install→새 Desktop PID 2372(parent staged updater 28512)→`exited`까지 성공했다. 실제 invalid Plugin 후보 E2E는 첫 strict Controller-handoff abort가 Desktop relaunch를 놓친 회귀를 발견했고, exact-root drain 및 모든 post-stop abort/rollback relaunch로 수정했다. 수정 뒤 `upd_iK7h…`는 integration file-set apply→postcheck failure→`rolled_back`→새 Desktop PID 1852(parent staged updater 15180)를 자동 완료하고 원래 Plugin file과 backup을 확인했다([E2E evidence](docs/testing/star-updater-restart-e2e-2026-07-18.md)). final stage는 x64 187 files·4 EXE·set `sha256:f2afe5a351f98253b8f96c732883e978eaf16d100c4f48d162b7bfbf67f0eb02`, ARM64 187 files·4 EXE·set `sha256:d856d923d7bb9ebd5f81e3692c638801cfc0e758845fc195c291a160d5ea958a`이며 두 stage verifier와 Inno installer compile을 통과했다. STRICT 봉인 검토에서 MCP EOF의 Controller 재기동, PID 순서 기반 child drain, creation-time 없는 PID 재사용 판정, `star.exe` drain 누락, task unknown의 0 승격, countdown mutation admission 누락을 수정했다. 최신 source FULL 10/10(`target/validation/20260720T085716996Z-28476/report.json`) PASS이며 final census에서 `star-updater` 0, 현재 `star-mcp → star-controller`가 `codex.exe → ChatGPT.exe` 소유 체인임을 확인했다. 공식 전체 task safe-point census는 public surface 부재로 instance/process-tree shutdown과 unknown task count로만 남고, native ARM64 installer 실행과 Hook trust 사용자 검토는 외부 경계다. Codex cache·trust·채팅 주입은 금지한다. |
...
| R-0050 | 살아 있는 task의 MCP를 Controller idle만으로 종료하면 다음 Tool 호출이 복구되지 않을 수 있음 | EOF와 verified Desktop owner death만 즉시 종료하고, 30초 lease는 Controller에만 적용 | installed Codex에서 MCP 종료 뒤 동일 task Tool 재호출이 새 Gateway를 생성한다는 E2E 증거 |

## 최신 검증 상태

- P-0039 CURRENT PASS — x64 4-EXE package에서 `restart_armed → exited`와 실제 integration file-set `apply → rollback → Desktop relaunch` E2E를 통과했다. STRICT 봉인 검토의 6개 lifecycle/process/admission 결함을 수정했고, latest source FULL은 49.8초 10/10 stable PASS(`target/validation/20260720T085716996Z-28476/report.json`)다. durable receipt·설치 hash·integration 상태는 [restart E2E evidence](docs/testing/star-updater-restart-e2e-2026-07-18.md)에 고정했으며 native ARM64 설치·실행은 환경 미제공 경계다.
...
- BOOTSTRAP v2 / RUNTIME E2E PASS — one-shot offline installer가 `D:\도구\Star-Control`에 Bootstrap v2를 설치했고 (`D:\Star-Control-MCP-Fix-Task-7da4d7ff-1fd0-4be8-ab50-9bfd88dcd4bc\installer-result.json`, exit 0), installed fixed `star-mcp.exe` SHA-256은 stage와 일치한다. Registry status·search·describe는 새 active Controller에 모두 성공했다. `rt_780b… → rt_9fe… → rt_780b… → rt_9fe…` 승인 기반 cutover가 모두 성공했고 activation revision 4의 `previous`는 rollback generation을 보존한다. 고정 MCP PID는 세 전환 내내 유지됐고 Controller PID/generation만 변경됐다.
...
- DOC PASS — 현재 Markdown 73개, restart E2E evidence를 docs index·P-0039에 연결했다. 최종 문서 link·diff Gate는 source 변경 뒤 다시 실행해야 한다.
