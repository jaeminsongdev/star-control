...
- `bundle_id`, `objective`, `scope_in`/`scope_out`, `depends_on`, `ownership`, `completion_criteria`, `validation`
- `authorization`: 새 task/thread 또는 parallel delegation의 명시 사용자 승인
- `thread_identity`: `list_projects` projectId, `list_threads`의 thread_id/host_id/worktree_root, setup-only clientThreadId
- `revision_identity`: baseline_sha, head_sha, diff_fingerprint
- `review_identity`: bundle_state, review_state, goal_status, blocked_reason
...
5. shared contract 필요, 소유권 충돌, 사용자 범위 변경은 Terra가 임의 수정하지 않고 Sol에 보고한다. Sol이 이미 유효한 결과를 보존한다.
