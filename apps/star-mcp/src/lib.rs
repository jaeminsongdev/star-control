...
impl Gateway {
    ...
    async fn dispatch(
        ...
        let mut controller_call =
            self.controller
                .call(command, mcp_tool, arguments, correlation_id.clone());
        let outcome = if let Some(context) = &context {
            let completed = tokio::select! {
                result = &mut controller_call => Some(result),
        ...
        let result = match outcome {
            ...
            Err(error) => McpToolResult {
                ...
                summary: "Controller IPC is unavailable.".to_owned(),
                data: None,
                operation_id: None,
                next_actions: vec![],
                artifact_refs: vec![],
                diagnostic_refs: vec![],
                error: Some(
                    serde_json::json!({"code":error.ipc_code(),"message":error.to_string(),"retryable":error.retryable()}),
                ),
...
async fn relay_stdio(relay: tokio::io::DuplexStream) -> io::Result<()> {
    ...
    loop {
        if !read_capped_line(&mut stdin, &mut line).await? {
            break;
        }
        let response = if line.len() > SUPERVISED_STDIO_MAX_LINE_BYTES {
            Some(protocol_error(
            ...
            std::str::from_utf8(&line)
                ...
                .map(|line| supervisor_response(line, &mut state))?
        };
...
async fn read_capped_line<R>(reader: &mut R, line: &mut Vec<u8>) -> io::Result<bool>
where
    R: tokio::io::AsyncBufRead + Unpin,
{
    ...
    loop {
        ...
        if line.len().saturating_add(take) > SUPERVISED_STDIO_MAX_LINE_BYTES {
            reader.consume(take);
...
fn supervisor_response(line: &str, state: &mut SupervisorState) -> Option<String> {
    ...
    if method == Some("initialize")
        && value
            ...
            .and_then(serde_json::Value::as_str)
            .is_some_and(|version| version != "2025-11-25" && version != "2025-06-18")
    {
        return Some(protocol_error(
            id,
            -32600,
            "Unsupported protocol version; supported version is 2025-11-25.",
        ));
    }
    match (*state, method) {
    ...
    {
        return Some(protocol_error(id, -32601, "MCP tool is not available."));
    }
    if method == Some("tools/call")
        && value
            .get("params")
...
fn fixed_arguments_valid(name: &str, arguments: serde_json::Value) -> bool {
    use star_contracts::fixed_mcp::{
        ApprovalResolveInput, CallInput, DescribeInput, OperationCancelInput, OperationGetInput,
        RegistryStatusInput, SearchInput,
    };

    match name {
        "star_tool_search" => serde_json::from_value::<SearchInput>(arguments).is_ok(),
        "star_tool_describe" => serde_json::from_value::<DescribeInput>(arguments).is_ok(),
        "star_tool_registry_status" => {
            serde_json::from_value::<RegistryStatusInput>(arguments).is_ok()
        }
        ...
        | "star_tool_call_destructive_open" => {
            serde_json::from_value::<CallInput>(arguments).is_ok()
        }
        "star_tool_operation_get" => serde_json::from_value::<OperationGetInput>(arguments).is_ok(),
        "star_tool_operation_cancel" => {
            serde_json::from_value::<OperationCancelInput>(arguments).is_ok()
        }
        "star_approval_resolve" => {
            serde_json::from_value::<ApprovalResolveInput>(arguments).is_ok()
        }
        _ => false,
    }
}
...
mod tests {
    ...
    async fn gateway_uses_the_fixed_wire_protocol_across_initialize_and_ping() {
        ...
        let mut state = SupervisorState::AwaitInitialize;
        let unsupported = supervisor_response(
            r#"{"jsonrpc":"2.0","id":14,"method":"initialize","params":{"protocolVersion":"2099-01-01"}}"#,
            &mut state,
        )
        .expect("unsupported version rejects before rmcp initialization");
        let unsupported: serde_json::Value = serde_json::from_str(&unsupported).unwrap();
        assert_eq!(unsupported["error"]["code"], -32600);
        assert!(
    ...
    fn gateway_preserves_fixed_result_status_and_compact_instructions() {
        ...
        let failure = mcp_call_result(McpToolResult {
            ...
            artifact_refs: vec![],
            diagnostic_refs: vec![],
            error: Some(serde_json::json!({"code":"X"})),
            correlation_id: RequestId::new(),
        });
    ...
    async fn stdio_line_reader_caps_unterminated_input_before_unbounded_allocation() {
        ...
        let mut line = Vec::new();
        assert!(read_capped_line(&mut reader, &mut line).await.is_err());
