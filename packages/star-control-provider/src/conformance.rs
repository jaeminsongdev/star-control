...
impl ProviderConformanceChecker {
    pub fn check_execution(
        ...
        if profile == ProviderConformanceProfile::Cloud {
            ...
            check_result_field(&cost_metric, "stage", result.stage())?;
        }
...
fn check_safe_segment(field: &str, value: &str) -> Result<(), ProviderConformanceError> {
    if value.is_empty() {
...
fn read_and_validate_json_artifact(
    context: &ProviderRunContext<'_>,
    job_id: &str,
    relative_path: &str,
    schema_file: &str,
...
mod tests {
    ...
    impl Fixture {
        ...
        fn context(&self) -> ProviderRunContext<'_> {
            ProviderRunContext::new(&self.registry, &self.store, &self.schema_root)
        }

        fn write_provider_json(&self, file_name: &str, value: &Value) {
            fs::write(
        ...
        fn write_provider_text(&self, file_name: &str, value: &str) {
            fs::write(self.provider_output_dir().join(file_name), value)
                .expect("write provider text");
        }
    ...
    fn execution_from_value(value: Value) -> ProviderExecution {
    ...
    fn result_value(provider_instance_id: &str, status: &str, artifacts: Vec<&str>) -> Value {
        json!({
            "schema_version": "1.0.0",
