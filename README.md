...
현재 repository는 구현 전 설계와 계약을 고정하는 단계다. v0 runtime 구현 스택은 Rust + Cargo workspace로 결정되었지만, 실제 앱, daemon, UI, runtime package, Cargo workspace 파일은 별도 구현 PR에서 추가한다.
...
- `packages/`: 목표 구현 package 경계. Cargo workspace 도입 전에는 구현 코드를 추가하지 않는다.
...
현재 단계는 스캐폴드와 설계 문서 상태다. 실제 앱, daemon, UI, Cargo workspace 파일은 아직 추가하지 않는다.
