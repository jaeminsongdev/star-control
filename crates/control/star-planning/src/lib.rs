...
use star_contracts::{
    ...
    ids::{ChangeSetId, ImpactAnalysisId, ScopeRevisionId, TaskSpecId, ValidationPlanId},
    index::{
    ...
    planning::{
        ...
        CheckDescriptor, CheckGraphV2, CheckInvocationTemplate, CheckOverride, CheckPlanV2,
        CheckResolutionOutcome, CollectionState, ExcludedScope, FULL_VALIDATION_PLAN_SCHEMA_ID,
        FallbackDecision, FullValidationPlan, GatePolicyV2, IMPACT_ANALYSIS_SCHEMA_ID,
        ImpactAnalysis, ImpactCertainty, ImpactConfidence, ImpactConfidenceSummary, ImpactEdge,
        ImpactKind, ImpactProjectInput, ImpactResolution, ImpactSeed, ImpactStatus, ImpactedNode,
        IntendedChange, NoResult, NoResultReason, ObservedChangeKind, PlanningBundle,
        PlanningContractError, PlanningSelector, ProjectTarget, ProjectTargetRole, ReviewKind,
        ReviewRequirementV2, RiskPathDescriptor, RiskPathFinding, RiskSeverityFloor,
        SCOPE_REVISION_SCHEMA_ID, ScopeApprovalState, ScopeAxis, ScopeItemSource, ScopeReasonCode,
        ScopeRelation, ScopeRevision, ScopeSet, ScopeSourceSnapshotRef, ScopedSelector,
        SeedResolution, SelectorKind, SuccessCriterion, TASK_SPEC_SCHEMA_ID, TaskSpec,
        ValidationPlanV2Readiness, ValidationRiskLevel, ValidationScopeLevel, document_ref,
        empty_fingerprint,
    },
};
...
fn analyze_impact(
    ...
    ImpactAnalysis {
        ...
        status: if limitations.iter().any(|code| code == "IMPACT_GRAPH_LIMIT") {
...
fn map_selector_to_seeds(
    ...
    let matched = match selector.kind {
        SelectorKind::Path => {
            let sources = project
                .source_entries
                .iter()
    ...
    let resolution = match matched.len() {
...
fn select_validation_plan(
    ...
    for changes in change_sets {
        for entry in &changes.entries {
            families.extend(families_for_source_class(entry.source_class));
        }
    }
    for risk in &impact.risk_paths {
        families.extend(risk.required_check_families.iter().cloned());
    }
    if families.is_empty() {
        families.insert("project_full".to_owned());
    }
    ...
    let descriptor_map = descriptors
        .iter()
        .map(|descriptor| (descriptor.family.as_str(), descriptor))
        .collect::<BTreeMap<_, _>>();
        ...
        .filter(|target| target.role != ProjectTargetRole::ReadOnlyImpact)
    {
        ...
        for family in &families {
            let Some(descriptor) = descriptor_map.get(family.as_str()).copied() else {
            ...
            .map_err(|_| PlanningError::Fingerprint)?;
            required_checks.push(CheckPlanV2 {
                ...
                outcome: CheckResolutionOutcome::SelectedRequired,
                reason_codes: vec!["AFFECTED_CHECK_SELECTED".to_owned()],
    ...
    let readiness = if unresolved_checks.is_empty() && !required_checks.is_empty() {
...
fn families_for_source_class(class: SourceClass) -> BTreeSet<String> {
    let families: &[&str] = match class {
        SourceClass::Source => &["format", "lint", "build", "test"],
        SourceClass::Test => &["test"],
        SourceClass::Docs => &["docs"],
        SourceClass::Config => &["config", "test"],
        SourceClass::Schema => &["contract", "test"],
        SourceClass::Migration => &["migration", "test"],
        SourceClass::Generated => &["generation", "build", "test"],
...
pub fn descriptor(
    ...
    let content_fingerprint = versioned_fingerprint(
        ...
        &serde_json::json!({
            "check_id":check_id,
            "family":family,
            "tool_id":"star.project.validator",
            "logical_executable":"project-validator",
            "args":args,
    ...
    Ok(CheckDescriptor {
        ...
        tool_id: "star.project.validator".to_owned(),
...
mod tests {
    ...
    use chrono::Utc;
    use star_contracts::{
        evidence::{ActorType, ArtifactRef},
        ids::{
    ...
    fn fixture() -> (ProjectCatalogSnapshot, PlanningProjectIndex, TaskSpecDraft) {
        ...
        let index = PlanningProjectIndex {
            ...
            entities: vec![provider, consumer],
            edges: vec![edge],
    ...
    fn task_to_impact_to_ready_validation_plan_is_deterministic_and_typed() {
        ...
        let descriptors = ["format", "lint", "build", "test"]
            .into_iter()
            .map(|family| {
                descriptor(
