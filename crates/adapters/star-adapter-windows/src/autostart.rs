//! Exact-owned current-user Run entry; no service or Task Scheduler support.
...
pub fn expected_command(controller: &Path) -> Result<String, AutostartError> {
    ...
    Ok(format!(
        "\"{}\" --background",
        normal_windows_path(&controller).display()
    ))
}
...
mod tests {
    ...
    #[test]
    fn expected_command_is_quoted_and_conflicts_fail_closed() {
        ...
        assert_eq!(
            decide_mutation(Some("foreign"), &command, true),
