...
- current installed runtime과 P-0055 candidate의 required core 17/17은 source·installed tree·Codex session을 각각 별도 증거로 판정한다. active Controller에서 installer를 겹쳐 실행하지 않고 disposable lifecycle 또는 updater close/restart 경계로 수집한다.
- P-0054/P-0055 dirty 변경은 local commit 전까지 immutable candidate가 아니므로 release profile이 `release-clean-worktree`에서 차단한다. clean revision을 만든 뒤에도 certificate·timestamp와 signed external evidence가 없으면 공개 Stable의 `blocked_external`을 유지한다.
