...
## P9. 공개 배포와 최종 완성 — P-0055 current host 17/17 완료, Runtime identity 보강, public blocked_external
...
- exact package 재생성 중 source revision만으로 generation ID를 만든 탓에 서로 다른 Runtime byte가 같은 ID를 받는 결함을 발견했다. 새 구현은 Controller·CLI Runtime·Catalog·Schema의 canonical file-set digest로 ID를 만들고, reseal payload 변화 시 generation directory를 바꾸며 verifier가 content ID를 재계산한다. identity 보강 source의 exact FULL/RELEASE·x64/ARM64 package·SBOM/audit/provenance·격리 lifecycle·GitHub draft/remote readback을 다시 봉인해야 하며, certificate·timestamp와 signed external evidence가 없으면 공개 Stable의 `blocked_external`을 유지한다.
