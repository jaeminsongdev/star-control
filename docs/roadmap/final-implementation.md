...
## M1. 읽기 전용 Project Catalog와 Code Index — P-0042 bounded Slice 구현
...
## M2. 변경 계획·영향 분석·affected 검사 선택 — P-0043 수직 Slice 구현

M2는 사용자가 지정한 **2단계 개발 관리 확장**이다. 의미 정본은 [변경 계획·영향 분석 계약](../contracts/change-planning-and-impact.md)이다. P-0043은 M1 persisted graph를 소비하는 `star-planning` pure engine, full planning contract·generated Schema·fixture, dirty workspace collector, idempotent global projection과 `planning create/get` Controller·CLI를 구현했다. P-0031/P-0035 `ValidationPlan` v1과 validator 실행 cache는 역사적 precursor이며 full plan의 실행은 M3가 소유한다.
...
## M4. 안전한 Patch·Refactor·codemod 엔진 — 첫 수직 Slice 구현

### 현재 상태

4단계 정본은 [안전한 Patch·Refactor·codemod 엔진 계약](../contracts/safe-patch-and-codemod.md)에 확정했다. P-0045는 built-in trailing-whitespace Recipe를 첫 실제 수직 Slice로 사용해 in-memory isolated preview, immutable sealed artifact, exact approval fingerprint, path·before/after hash TOCTOU guard, same-directory atomic replace와 exact rollback을 구현했다. `patch.prepare/apply`는 Project partition과 current source snapshot을 다시 확인하며 unrelated dirty file을 보존한다.

이 Slice는 external codemod worktree, 모든 selector와 PatchSet v2 migration 전체를 구현했다고 주장하지 않는다. 해당 확장은 M11 Rust adapter와 후속 Recipe가 같은 공통 경계를 사용한다.
...
5단계 정본은 [관리형 Symbol·상수·에러 코드 Registry 계약](../contracts/managed-symbol-registry.md)과 [ADR-0009](../decisions/ADR-0009-Git-정본-Managed-Registry와-Patch-Gate-경계.md)에 확정했다. P-0046 bounded Slice는 Git source manifest loader, `ManagedRegistrySnapshot`, exact consumer migration plan, 공개 Schema·fixture와 stale consumer·alias omission 차단을 `star-development`에 구현했다. source manifest를 역으로 쓰는 DB 경로, 무승인 generator/codemod와 cross-project apply는 구현하지 않았다.
...
6단계 정본은 [계약 호환성·문서·설정·개발 환경 관리](../contracts/contract-compatibility-and-environment.md)에 확정했다. P-0047 bounded Slice는 API·Schema·config·docs baseline comparator, 공개 contract type·Schema·fixture와 non-mutating `CleanRoomDoctorReport` evaluator를 `star-development`에 구현했다. doctor는 missing probe를 pass로 만들지 않으며 download·install·system/source write를 수행하지 않는다. 실제 provider별 clean VM 생성과 외부 package 설치는 이 Slice에 포함하지 않는다.
...
7단계 정본은 [실패 재현·보안·의존성 유지보수 계약](../contracts/failure-security-and-dependency-maintenance.md)에 확정했다. P-0048 bounded Slice는 normalized failure family, `ReproductionPack`, dependency/security/supply-chain limitation과 deterministic `MaintenanceRadar`, 공개 Schema·fixture를 `star-development`에 구현했다. 외부 관찰이 missing/stale이면 partial limitation을 보존한다. debugger·scanner·package-manager·network client를 재구현하거나 live dependency update를 수행하는 경로는 포함하지 않았다.
...
## M8. 데이터·설정·DB migration, 성능·build, 언어·플랫폼 migration — P-0049 bounded Slice 구현
...
## M10. CI·Release·평가·최종 제품 완성 — P-0051 구현, P-0053 local audit 완료
...
- P-0041~P-0050은 M1~M9의 첫 bounded 제품 Slice와 required core source surface를 구현했다. P-0051은 M10 `ReleaseManifest` v2·`EvaluationRun` v2·Catalog lifecycle·build-once pure engine과 fake provider fault corpus를 구현했다. P-0053은 P-0026 technical manifest/installer와 M10 상태기계를 연결해 local release build·lifecycle·ARM64 simulation·signed-stage verification 경계를 감사했다. 실제 signer·clean signed installer lifecycle·GitHub remote effect는 외부 Gate다.
...
## M11. Rust 코드 스타일 자동 교정 Profile — P-0052 bounded 제품 Slice 구현

### 현재 상태와 선행 blocker

- [Rust 코드 스타일 자동 교정 정본](../features/rust-code-style-auto-fix.md)과 [ADR-0011](../decisions/ADR-0011-Stable-rustfmt-Allowlisted-Clippy-Personal-Auto-경계.md)에 따라 P-0052가 Rust nested type·Schema·fixture, fixed cargo/rustfmt/Clippy adapter, immutable PatchSet pipeline, exact approval·`personal_auto`, recovery와 multi-crate Corpus를 구현했다. 실제 사용자 checkout apply와 background automation은 수행하지 않으며 CLI surface 확장은 후속 bounded Slice다.
- root `Cargo.toml`과 `rust-toolchain.toml`은 workspace edition 2024와 exact Rust `1.96.0`, rustfmt·Clippy component를 current source로 고정한다. installed executable identity와 component availability는 실행마다 별도 관찰하며 source pin만으로 available을 추측하지 않는다.
...
## P9. 공개 배포와 최종 완성 — P-0053 local audit 완료, candidate BLOCK/public blocked_external

### 현재 상태

- Windows runtime·Plugin package의 P-0026 transport, architecture별 stage·Inno Setup installer·installation record·실제 경로 렌더링과 P-0051 M10 build-once/status/publish-after-state pure engine을 구현했다. P-0053은 x64 native build·격리 lifecycle, ARM64 Preview cross-build·PE/file manifest·installer model·fake lifecycle과 signed-stage fail-closed 재봉인을 검증했다. current signed candidate의 clean installer lifecycle과 GitHub publication은 아직 실행하지 않았다.
- final 16번째 `rust_style_auto_fix`는 P-0052에서 fixed adapter·immutable PatchSet·exact approval·idempotence·multi-crate Corpus까지 구현·검증했다. 실제 사용자 checkout apply는 exact current binding·approval·pre/post Gate가 있는 별도 실행이며 public release blocker와 혼동하지 않는다.
...
- current installed runtime은 과거 6-action build이므로 source의 required core 17/17 readiness를 installed 17/17 evidence로 승격하지 않는다. active Controller를 임의 중단하지 않고 source candidate를 설치하는 clean signed lifecycle 뒤 Codex·Inspector 17/17을 다시 수집한다.
- P-0040~P-0053 dirty 변경은 local commit 전까지 immutable candidate가 아니므로 release profile이 `release-clean-worktree`에서 차단한다. clean revision을 만든 뒤에도 certificate·timestamp와 signed external evidence가 없으면 `blocked_external`을 유지한다.
