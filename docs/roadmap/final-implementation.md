...
- P-0055 staged-tree candidate는 official Inspector fixed 12/12·required core search/describe 17/17을 통과했다. current installed runtime은 registry revision 4의 ready core 6개이므로 source·candidate Inspector·installed Codex session을 계속 별도 증거로 판정한다. active Controller에서 installer를 겹쳐 실행하지 않고 disposable lifecycle 또는 updater close/restart 경계로 수집한다.
- P-0055 final code index tree는 `2eb3680b3f0cf5a8ae6b0daadff6fe54f003e067`이고 FULL 10/10, x64/ARM64 package·simulation·SBOM/audit·GitHub draft byte roundtrip은 통과했다. 그러나 host가 `git commit`을 거부해 immutable revision이 아니며 RELEASE는 clean-worktree 1 fail, signing/publication 1 unverified다. clean revision을 만든 뒤에도 certificate·timestamp와 signed external evidence가 없으면 공개 Stable의 `blocked_external`을 유지한다.
