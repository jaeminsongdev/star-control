...
## P9. 공개 배포와 최종 완성 — P-0055 exact local/remote candidate 봉인, current host pending, public blocked_external
...
- P-0055 exact local candidate `b20d234b38a7dcb347049b6b95aff3407c5dedc9`는 official Inspector fixed 12/12·required core search/describe 17/17과 Hook-correct `validation.run` FULL 10/10을 통과했다. current installed runtime은 registry revision 4의 ready core 6개이므로 source·candidate Inspector·installed Codex session을 계속 별도 증거로 판정한다. active Controller에서 installer를 겹쳐 실행하지 않고 disposable lifecycle 또는 updater close/restart 경계로 수집한다.
- implementation tree `2eb3680b3f0cf5a8ae6b0daadff6fe54f003e067`은 commit `4554c4a`로, 제품 증거 기준은 docs commit `b20d234`로 봉인했다. exact revision FULL 10/10과 x64/ARM64 package·simulation·SBOM/audit·pre-sign provenance는 통과했고 RELEASE는 clean-worktree를 포함해 14/15 PASS, signing/publication만 1 unverified다. origin exact source readback과 exact-target GitHub draft asset upload/download digest reconcile·release/tag cleanup도 통과했다. current Codex restart install만 별도 비서명 Gate이며, certificate·timestamp와 signed external evidence가 없으면 공개 Stable의 `blocked_external`을 유지한다.
