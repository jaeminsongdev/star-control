...
| MCP/core | required core 17개 중 6개 action 구현, 남은 11개 owner·Schema와 current runtime evidence | safe_default first-run·제품 완성 block |
...
- root `Cargo.toml`은 workspace, edition 2024, `rust-version = "1.96"`이지만 현재 repository에 `rust-toolchain.toml`은 없다. final layout의 목표 파일을 current pinned toolchain으로 간주하거나 M11 문서 작업에서 만들지 않는다.
...
- Windows runtime·Plugin package의 P-0026 transport, architecture별 stage·Inno Setup installer·installation record·실제 경로 렌더링은 구현됐다. M10 build-once 승격, approved state, clean x64 Stable install lifecycle, ARM64 Preview simulation과 publish after-state engine은 없다.
