...
## M2. 변경 계획·영향 분석·affected 검사 선택 — 설계 확정, 구현 전

M2는 사용자가 지정한 **2단계 개발 관리 확장**이다. 의미 정본은 [변경 계획·영향 분석 계약](../contracts/change-planning-and-impact.md)이다. 현재 반영은 문서 설계뿐이며 planner, graph traversal, affected selector, test runner, CLI command, Schema와 DB migration을 구현하지 않았다.
...
계약 의미와 설정 병합 설계는 [데이터 계약 지도](../contracts/README.md), [ADR-0002](../decisions/ADR-0002-데이터-계약과-설정-정본.md), [ADR-0004](../decisions/ADR-0004-무재시작-고정-MCP와-Live-Tool-Registry.md)와 [ADR-0005](../decisions/ADR-0005-MCP-구현-계약-동결.md)로 확정했다. MCP exact field·hash·Win32 순서·검증 행렬의 Rust type, generated Schema, fixture와 runtime 수직 Slice는 구현됐다. 과거 official MCP Inspector와 native ARM64 Windows 11 25H2 build 26200 evidence는 보존돼 있지만 2026-07-12 독립 감사에서 current 통합 binary의 required core 13개 owner·Schema 부재, current Inspector core search 실패와 native ARM64 evidence stale가 확인됐다. exact 24H2 baseline도 미검증이므로 P1 전체 판정은 **BLOCK**이며 완료로 표시하지 않는다. 세부 판정은 [MCP 독립 감사](../testing/mcp-independent-audit-2026-07-12.md)와 [MCP 완료 감사](../testing/mcp-completion-audit.md)를 따른다. P1의 아직 미구현 공통 계약과 P2 이후 도구는 P0 관리 계약을 소비하며 별도 DB type을 만들지 않는다.
...
사용자가 지정한 3단계 `M3 공통 검증·품질 Gate`는 [상세 설계](../features/common-validation-gate.md), [검사·증거 계약](../contracts/validation-and-evidence.md), [오류·Diagnostic](../contracts/errors-and-diagnostics.md)과 [설정·Validator Registry](../contracts/config-and-catalog.md)에 목표 계약을 확정했다. 현재 작업은 **문서 설계뿐**이며 validator, test runner, Corpus, Rule·Baseline·Suppression·Disposition·Diagnostic migration, DB Schema, CLI와 제품 code를 구현하지 않았다.
...
## M4. 안전한 Patch·Refactor·codemod 엔진 — 설계 확정, 구현 전

### 현재 상태

4단계 정본은 [안전한 Patch·Refactor·codemod 엔진 계약](../contracts/safe-patch-and-codemod.md)에 확정했다. `refactor_codemod`, A04·A08·A09·A10, B01·B02·B04, evidence·Tool Registry·권한·worktree·Package 경계를 함께 동기화했다. 현재 작업은 **문서 설계뿐**이며 source rewrite, codemod 실행, worktree 생성, file mutation, contract type·Schema·migration, CLI와 제품 code를 구현하지 않았다.

P0의 ChangeRecipe·ChangePlan·PatchSet 첫 수직 Slice는 historical 기반으로 유지하지만 M4의 typed selector, rewrite assurance, RecipeExecution, PatchSet v2, PatchApplication, idempotence, partial recovery와 CLI-only flow가 구현됐다는 근거가 아니다.
...
## M5. 관리형 Symbol·상수·에러 코드 Registry — 설계 확정, 구현 전

### 현재 상태

5단계 정본은 [관리형 Symbol·상수·에러 코드 Registry 계약](../contracts/managed-symbol-registry.md)과 [ADR-0009](../decisions/ADR-0009-Git-정본-Managed-Registry와-Patch-Gate-경계.md)에 확정했다. 현재 작업은 **문서 설계뿐**이며 `.star-control/managed-registry` file, contract type·Schema·DB migration, scanner 확장, generator, codemod, CLI와 실제 error/config/constant 변경을 구현하지 않았다.
...
## M6. API·계약·문서·설정·개발 환경 관리 — 설계 확정, 구현 전

### 현재 상태

6단계 정본은 [계약 호환성·문서·설정·개발 환경 관리](../contracts/contract-compatibility-and-environment.md)에 확정했다. 현재 작업은 **문서 설계뿐**이며 `.star-control/contracts.toml`, 8개 contract type·Schema, comparator, docs/config validator, project doctor, environment probe, clean-room runner와 CLI command를 구현하지 않았다.
...
## M7. 실패 재현·보안·의존성 유지보수 — 설계 확정, 구현 전

### 현재 상태

7단계 정본은 [실패 재현·보안·의존성 유지보수 계약](../contracts/failure-security-and-dependency-maintenance.md)에 확정했다. 현재 작업은 **문서 설계뿐**이며 Failure/Reproduction/Dependency/SupplyChain/Radar type·Schema, debugger·scanner·package-manager adapter, updater, network client, external-data cache와 실제 update code를 구현하지 않았다.
...
## M8. 데이터·설정·DB migration, 성능·build, 언어·플랫폼 migration — 설계 확정, 구현 전

### 현재 상태

8단계 정본은 [Migration·성능·언어·플랫폼 계약](../contracts/migration-performance-and-platform.md)에 확정했다. 현재 반영은 **문서 설계뿐**이며 migration/measurement/language contract type·Schema, project manifest, checkpoint runner, backup/restore adapter, benchmark, profiler, compiler, codegen/codemod, DB migration, target OS validation과 CLI를 구현하거나 실행하지 않았다.
...
9단계 local Git/worktree/merge 의미는 [CrossRepo ChangeBundle 정본](../contracts/cross-repo-change-bundle.md)과 [병렬 작업과 병합](../architecture/worktrees-and-merge.md)에 확정했다. 현재 반영은 **문서 설계뿐**이며 WorktreeRecord·MergePlan v2·queue/conflict/result type·Schema, Git mutation adapter, worktree 생성, branch·commit·merge, CLI와 Corpus를 구현하거나 실행하지 않았다.
...
P7은 사용자 9단계의 global/project coordination과 remote 경계를 완성한다. [CrossRepo ChangeBundle 정본](../contracts/cross-repo-change-bundle.md)의 `MultiProjectGoal`, `CrossRepoChangeBundle`, `ChangeBundleParticipant`, `RemoteOperationRecord`, `ChangeBundleReleaseHandoff`가 구현 기준이다. 현재는 문서 설계뿐이며 cross-repo write, remote upload·PR·merge·publish를 실행하지 않았다.
...
## M10. CI·Release·평가·최종 제품 완성 — 설계 확정, release/evaluation 구현 전
...
- P0 첫 수직 Slice, MCP 수직 Slice와 P-0026 Windows 설치 transport Slice가 구현됐다. M1~M9 제품 Gate와 M10 release/evaluation type·engine·CLI·Corpus·provider adapter는 구현 전이다. P-0026의 technical manifest Schema와 installer는 M10 `ReleaseManifest`·ready 상태기계를 대신하지 않는다.
...
| MCP/core | required core 13개 owner·Schema와 current runtime evidence | safe_default first-run·제품 완성 block |
...
## M11. Rust 코드 스타일 자동 교정 Profile — 설계 확정, 구현 전

### 현재 상태와 선행 blocker

- [Rust 코드 스타일 자동 교정 정본](../features/rust-code-style-auto-fix.md)과 [ADR-0011](../decisions/ADR-0011-Stable-rustfmt-Allowlisted-Clippy-Personal-Auto-경계.md)로 제품 의미만 확정했다. Rust type·Schema·fixture·Catalog TOML·adapter·CLI·Corpus·제품 code는 아직 구현되지 않았다.
- root `Cargo.toml`은 workspace, edition 2024, `rust-version = "1.85"`이지만 현재 repository에 `rust-toolchain.toml`은 없다. final layout의 목표 파일을 current pinned toolchain으로 간주하거나 M11 문서 작업에서 만들지 않는다.
...
- M1 current Cargo graph/source ownership, M2 accepted scope·impact·affected Check, M3 candidate/pre/post Gate·Evidence와 M4 isolated immutable PatchSet/SourceMutationPort가 실제 type·Schema·fixture·CLI-only E2E로 통과하기 전에는 M11 source mutation Slice를 시작하지 않는다.
- read-only discovery pure function을 먼저 만들 수는 있으나 fake M1~M4 success, historical P0 Patch/Validation과 문서 존재로 apply Gate를 대체하지 않는다.

### 제품 구조 불변식

- core 기능 수는 23개, runtime executable은 `star.exe`, `star-controller.exe`, `star-mcp.exe` 3개를 유지한다.
...
M11 구현 완료는 모든 case가 current tool/config/source/Catalog fingerprint에서 통과하고 EvidenceBundle v6·ReviewPack이 complete일 때만 표시한다. 문서 완료, process exit 0, partial coverage와 x64-only 결과는 구현 완료가 아니다. M11 implementation/conformance가 완료되기 전 P9 release `ready`를 만들지 않는다.

## P9. 공개 배포와 최종 완성

### 현재 상태

- Windows runtime·Plugin package의 P-0026 transport, architecture별 stage·Inno Setup installer·installation record·실제 경로 렌더링은 구현됐다. M10 build-once 승격, approved state, clean x64·native ARM64 전체 install lifecycle과 publish after-state engine은 없다.
- final 16번째 `rust_style_auto_fix`는 M11 설계만 확정됐고 제품 conformance가 없다. M11 완료 evidence 없이는 P9 final Profile audit와 release `ready`를 차단한다.
- 지원 target 의도는 Windows 11 24H2 build 26100 이상 x64·ARM64다. native ARM64·clean installer·update/rollback/uninstall evidence는 외부 gate다.
- installer는 current-user Inno Setup 6 `.exe`로 확정했다. portable 공개 지원과 public channel의 SBOM·provenance·code signing 필요성은 공개 구현 직전 최신 공식 자료와 실제 배포 환경으로 확정한다.
...
7. 동일한 native ARM64 install·runtime·update·rollback·uninstall matrix를 통과한다. cross-build만으로 대체하지 않는다.

### ready·approval·publish Slice

1. pure status reducer에서 `draft -> candidate -> ready -> approved -> publishing -> published`를 구현한다.
2. `blocked`, `publish_outcome_unknown`, `rollback_required`, `withdrawn`을 독립 상태로 유지한다.
3. approval을 exact manifest revision·artifact digest·channel·provider·destination·expiry·before snapshot에 bind한다.
4. publish·deploy·withdrawal·rollback을 서로 다른 RemoteOperationRecord와 승인으로 실행한다.
...
7. deploy smoke·observation window·rollback trigger를 publish 전에 versioned policy로 고정한다.
...
- clean Windows x64·native ARM64에서 build·test·package·install·safe_default first-run·update·rollback·uninstall이 실제 통과한다.
...
- publish·deploy·withdrawal·rollback이 action별 명시적 승인과 before/after snapshot을 가진다.
...
- M11 `rust_style_auto_fix`의 stable toolchain·exact allowlist·coverage·isolated PatchSet·idempotence·`personal_auto`·Windows x64/ARM64 CLI-only conformance가 EvidenceBundle v6으로 complete하다.
