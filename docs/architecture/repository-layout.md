...
문서 폴더 migration은 완료됐다. 제품 Package와 코드는 아직 만들지 않으며, [최종 구현 로드맵](../roadmap/final-implementation.md)의 P1부터 필요한 부분만 생성한다.
...
- 사용자 설정은 TOML, 상태와 증거는 version이 있는 JSON·JSONL을 사용한다.
...
최종 runtime code는 3개 실행 파일과 21개 내부 Package로 구성한다. Package 수를 기능 수와 맞추지 않고, 9개 검증 기능은 `star-checks` module로, 15개 작업 유형은 `catalog/profiles` data로 흡수한다.
...
│     ├─ commands/                  # start, plan, status, pause, resume, tools validate·trust 등
...
│     ├─ bootstrap/                 # config·catalog·live Registry·adapter 조립
...
| 실행 파일 | 담당 | 넣지 않는 것 |
|---|---|---|
| `star.exe` | 사람이 쓰는 terminal 명령과 표시 | 상태 직접 수정, 계획·권한 판단, Git 직접 실행 |
| `star-controller.exe` | 상태의 단일 writer와 전체 use case 조립 | 사용자 UI, 자체 AI 호출, HTTP API server |
| `star-mcp.exe` | 고정 MCP surface와 Controller IPC 변환 | TOML·Registry·tool별 handler·EXE path·parser, 별도 상태·정책, Codex App Server 직접 제어 |
...
├─ star-contracts/
│  └─ src/
│     ├─ ids.rs                     # Goal·Stage·Run·Attempt·Artifact ID
...
│     ├─ repository/                # typed state repository
...
└─ star-application/
   └─ src/
      ...
      │  ├─ recover.rs
      │  └─ export.rs
      ├─ queries/                   # status·plan·evidence·capability 조회
...
- engine에서 `std::fs`, raw process 실행, network client 또는 Git command 직접 호출
...
│  └─ retention.toml
├─ validators/
│  ├─ registry.toml                 # B01~B09 descriptor
...
```text
.star-control/
├─ config.toml                      # 이 repo의 승인·병렬·보존 기본값
...
이 폴더에는 사용자 secret, 절대 경로, 개인 비용 한도를 넣지 않는다. 개인 설정은 `%APPDATA%\Star-Control\config.toml`에 둔다.
...
| runtime 상태 | `%LOCALAPPDATA%`, 대상 repo `.ai-runs/` | source release와 분리 |
...
| A03 프로젝트 이해·Context Pack | `star-project` | `star-ports`, `catalog/profiles` |
| A04 변경 영향·위험 | `star-project/impact·risk_paths` | `star-checks/change_scope`, `star-adapter-git` |
...
| A07 상태·Checkpoint·자체 복구 | `star-state` | `star-execution/checkpoint·recovery`, `star-evidence` |
...
| B01 diff·범위·주장·증거 | `star-validation/claim·gate·review_pack` | `star-checks/change_scope`, `star-evidence` |
...
| B04 계약·구조·설정·migration | `star-checks/contract_architecture` | `star-project`, `specs/`, `star-config` |
...
| P3 | `star-planning`, `star-project`, `star-routing`, `star-policy` |
| P4 | `star-execution`, `star-state`, `star-evidence`, Codex·Windows adapter, `star-application` |
| P5 | `star-validation`, `star-checks`, `corpus/`와 관련 tests |
...
각 단계는 다음 단계가 실제로 필요로 하는 공개 계약까지만 먼저 만든다. 미래 Package의 빈 폴더와 사용되지 않는 추상화는 만들지 않는다.
...
6. 계약·설정·Profile·정책·증거와 생성 문서의 정본이 각각 하나다.
7. built-in 검사군은 공통 diagnostic·gate·Review Pack 계약을 사용한다.
8. Package 의존 graph에 cycle과 모호한 공용 Package가 없다.
9. clean Windows에서 설치, 첫 실행, update, rollback, uninstall을 검사할 수 있다.
10. 현재 문서만으로 source, runtime 상태, 생성물과 local-only legacy 경계를 이해할 수 있다.
