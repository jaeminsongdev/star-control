...
| M7 Daemon / API | CLI approve/cancel/resume regression, daemon queue skeleton test, API read-only service test, daemon/API smoke와 resume/cancel/approval regression test를 추가한다. |
| M8 UI Shell | `star-control-ui` view model contract, read-only no-write smoke, approval path smoke를 추가한다. |
