...
- Bootstrap Bridge v1→v2 최초 설치는 offline installer만 수행한다. installer는 `installation finalize` 다음에 `installation bridge initialize --state-generation bootstrap_v2`를 실행한다. direct installer update는 Codex·MCP가 실행 중이면 파일 변경 전 중단하고, 실행 중 host의 full/mixed 교체는 `star update offline-installer-restart`의 exact census·10초 countdown·detached Updater만 수행한다. routine Runtime-only update는 Codex/MCP 재시작 없이 `star-updater.exe`가 수행한다.
- 기존 v2 설치를 full/mixed installer로 교체한 뒤에는 installer exit code만 보지 않는다. Updater가 새 root manifest 소유 generation을 정확히 하나 선택해 activation하고, candidate manifest ToolId 전체가 live release Registry에서 모두 declared·ready인지 exact set으로 확인한다. 실패하면 prior activation을 복원하되 replacement files가 남는 `partially_applied`와 selector 복구 자체가 실패한 `rollback_failed`를 구분한다.
- 이미 설치된 manifest-owned Runtime만 stale selector 때문에 비활성이라면 `star update reconcile-installed-runtime --install-root <absolute-path> --json`을 먼저 사용한다. 이 경로는 Codex/MCP를 재시작하거나 fixed payload를 교체하지 않는다. update lease, manifest-owned 단일 generation, expected ToolId, installed trusted CLI의 declared/ready exact set과 integration verified를 요구하며, 실패 시 prior selector를 복구한다. exact prior Controller가 12초 뒤에도 남은 경우에만 해당 image tree를 종료하고 PID를 결과에 기록한다.
- 중단된 restart receipt가 apply 전 `draining` 등에 남았더라도 직접 JSON을 고치지 않는다. reconcile이 update lease를 확보한 뒤 동일 install root의 pre-apply receipt만 `aborted`로 종결한다. `applying` 이후와 partial/unknown 상태는 자동 승격·재분류하지 않는다.
