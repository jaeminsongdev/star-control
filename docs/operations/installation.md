...
주 배포 방식은 **installer-first**다. portable archive를 보조 산출물로 제공할지는 후속 호환성 검증에서 결정하며 installer와 동일한 lifecycle을 검증하지 않은 archive를 동등한 주 배포 방식으로 표시하지 않는다. 구체 installer 기술은 P9 구현 직전 Windows x64·ARM64, code signing, current-user install, update·rollback·uninstall 공식 지원을 조사해 선택한다.
