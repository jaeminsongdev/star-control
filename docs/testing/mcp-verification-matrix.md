...
공식 MCP Inspector와 protocol JSONL fixture 둘 다 통과해야 한다. SDK 자체 test만으로 대체하지 않는다.
...
2026-07-12 실제 Codex CLI MCP host 검증 결과는 `apps/star-mcp/tests/evidence/codex-same-session-v1.json`에 정규화해 고정했다. C001~C007은 한 Codex turn과 동일한 Codex·Gateway·Controller PID에서 수행했다. C008은 Gateway 종료 직후 같은 turn의 `Transport closed`를 먼저 확인하고, 동일 Codex task ID를 resume해 새 Gateway가 연결된 뒤 Controller instance·Registry revision·terminal Operation이 보존됨을 확인했다. `tools/codex-e2e-fixture.ps1`은 네트워크·유료·원격 동작 없이 이 fixture 변경과 PID 증거를 재현한다.
