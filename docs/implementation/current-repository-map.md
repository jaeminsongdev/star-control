...
| `packages/` | `CANONICAL` / `SCAFFOLD` | `star-control-*` Cargo workspace crate와 `star-sentinel` 구현 코드를 둔다. `star-control-api`는 read-only service까지 구현하며 HTTP server/mutation은 아직 reserved다. 기존 provider/transport/adapter scaffold는 post-core 확장 후보로 남긴다. |
...
| CLI / daemon queue / API read-only / reserved surfaces | `cli-command-reference.md`, `daemon-contract.md`, `api-contract.md`, `ui-shell-contract.md` | `CANONICAL` / `RESERVED` |
...
| daemon process/API mutation/UI | daemon queue skeleton과 API read-only service 이후에도 process, API server/mutation, UI는 별도 slice까지 `RESERVED`다. |
