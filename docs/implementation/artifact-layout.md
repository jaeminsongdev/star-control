...
`audit/audit-events.jsonl`은 감사 목적의 AuditEvent JSONL log다. CoreEvent `events.jsonl`이 execution timeline이라면, AuditEvent는 사람이 나중에 검토해야 하는 control-plane 사건의 요약이다.

M9b 기준 writer는 `packages/star-control-observability`의 `AuditEventWriter`이며, `specs/schemas/audit-event.schema.json`을 만족하는 event만 append한다. 저장 전 shared redaction utility를 적용하고, raw secret/token/credential value를 남기지 않는다.
...
`release/release-readiness.json`은 release/deploy/publish 자동화 전에 필요한 ReleaseReadiness artifact다.

M9f 기준 writer는 `packages/star-control-release`의 `ReleaseReadinessWriter`이며, `specs/schemas/release-readiness.schema.json`을 만족하는 artifact만 저장한다. 현재 slice에서는 `ready` status를 거부하고, `reserved` status에는 blocker explanation을 요구한다.

실제 GitHub release, package publish, deploy, signing, repository settings 변경은 이 artifact만으로 실행하지 않는다. 해당 작업은 별도 승인과 control-plane integration이 생긴 뒤에만 수행한다.
