...
Canonical vocabulary는 `docs/implementation/handoff-vocabularies.md`와 schema enum을 따른다.
...
| `public_api_change` | `HIGH` | `full` | true | `HUMAN_REVIEW` |
| `credential_change` | `CRITICAL` | `security` | true | `HUMAN_REVIEW` or `BLOCK` |
| `sensitive_data_exposure` | `CRITICAL` | `security` | true | `BLOCK` |
...
| `risk_path_change` | `HIGH` | `full` | true | `HUMAN_REVIEW` |
| `external_account_change` | `CRITICAL` | `security` | true | `HUMAN_REVIEW` or `BLOCK` |
| `budget_exceeded` | `HIGH` | `full` | true | `HUMAN_REVIEW` |
| `unknown_high_risk` | `HIGH` | `full` | true | `HUMAN_REVIEW` |
...
| 민감정보 노출, out-of-scope, validator self-bypass, destructive action | `BLOCK` |
...
4. sensitive data exposure -> `CRITICAL`, `security`, `BLOCK`
