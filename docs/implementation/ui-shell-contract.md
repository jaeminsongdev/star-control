...
UI shell은 장시간 작업 관제를 사람이 쉽게 볼 수 있게 하는 장기 surface다. M8a에서는 read-only view model을 `packages/star-control-ui`에 library-level로 구현하고, browser app은 후속 slice로 남긴다.
...
초기 UI는 read-only view model부터 시작한다. 승인/취소/재개 mutation은 API와 CLI 안정화 이후 추가한다.
