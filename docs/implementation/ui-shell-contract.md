...
UI shell은 장시간 작업 관제를 사람이 쉽게 볼 수 있게 하는 장기 surface다. M8a에서는 read-only view model을 `packages/star-control-ui`에 library-level로 구현하고, M8b에서는 browser-oriented control shell model을 같은 crate에 구현한다. Productization slice E51은 `apps/star-control-ui` 정적 browser app으로 `star-daemon api`를 소비한다.
...
daemon state viewer
job list/detail/timeline viewer
release readiness viewer
approve/cancel/resume action panel
Local/Cloud AI connector disabled surface
Node 내장 test runner 기반 helper regression test
```
