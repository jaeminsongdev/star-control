...
- `docs/decisions/0005-full-implementation-defaults.md`의 기본 Rust dependency set은 구현 PR에서 목적, version, 검증 결과를 기록하는 조건으로 사용할 수 있다.
- 기본 Rust dependency set 밖의 새 production dependency, Cargo 외 package manager, 배포, 릴리즈, 외부 계정 변경은 명시 승인 전까지 하지 않는다.
...
- `docs/decisions/0005-full-implementation-defaults.md`의 기본 Rust dependency set 밖의 새 production dependency 추가가 필요해 보일 때
...
- `docs/decisions/0005-full-implementation-defaults.md`의 기본 Rust dependency set을 목적, version, 검증 결과와 함께 추가하는 구현 PR
...
새 세션은 반드시 `AGENTS.md`, `README.md`, `docs/implementation/README.md`, `docs/decisions/0002-runtime-stack.md`, `docs/decisions/0005-full-implementation-defaults.md`, `docs/implementation/codex-long-run-workflow.md`, `docs/implementation/codex-work-queue-current.md`, 현재 EPIC 문서를 다시 읽는다.
...
단, `dependency_addition` 중 `docs/decisions/0005-full-implementation-defaults.md`의 기본 Rust dependency set을 Star-Control Rust workspace 구현 PR에서 추가하는 경우는 예외다. 이때도 dependency 목적, version, 실행한 validation command는 PR 본문과 작업 기록에 남긴다.
