...
현재 착수 큐의 M9a는 E19-T01을 `packages/star-control-security`의 shared redaction utility와 RedactionReport builder로 구현한다. M9b는 E19-T05의 기반을 `packages/star-control-observability`의 AuditEventWriter와 `audit/audit-events.jsonl` writer로 구현한다. M9c는 E19-T04의 기반을 같은 crate의 CostMetricWriter와 warning-only budget evaluation으로 구현한다. API/CLI/daemon/provider audit/cost integration, hard budget enforcement, retention/recovery, release readiness는 후속 M9 slice로 남긴다.
