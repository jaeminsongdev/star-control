...
현재 착수 큐의 M9a는 E19-T01을 `packages/star-control-security`의 shared redaction utility와 RedactionReport builder로 구현한다. M9b는 E19-T05의 기반을 `packages/star-control-observability`의 AuditEventWriter와 `audit/audit-events.jsonl` writer로 구현한다. M9c는 E19-T04의 기반을 같은 crate의 CostMetricWriter와 warning-only budget evaluation으로 구현한다. M9d는 `packages/star-control-provider`의 ProviderConformanceChecker hardening으로 result/ref/file/schema 일치를 검증한다. M9e는 `packages/star-control-state`의 inspect-only recovery report로 missing/corrupt/tmp artifact를 분류한다. M9f는 `packages/star-control-release`의 ReleaseReadinessWriter로 `release/release-readiness.json` writer를 추가한다. M9g는 `packages/star-control-api`의 ApiReadOnlyService release readiness endpoint로 existing readiness artifact를 읽는다. M9h는 `packages/star-control-release`의 ReleaseConsistencyChecker로 version/changelog checks와 blockers를 만든다. M9i는 `packages/star-control-release`의 ReleaseEvidenceFileChecker로 project root 내부 evidence file을 read-only로 읽는다. M9j는 같은 crate의 ReleaseProfileValidation과 ReleaseProfileReadinessBuilder로 release profile result를 readiness checks에 병합한다. M9k는 `packages/star-control-ui`의 UiReadOnlyShell release readiness viewer로 API read surface를 UI read model에 연결한다. M9l는 `packages/star-control-cli`의 `report --release-readiness` option으로 existing readiness artifact를 CLI read surface에 연결한다. M9m은 `packages/star-control-release`의 ReleaseReviewPackWriter로 `review-packs/release-review-pack.md` Markdown artifact를 생성한다. API/CLI/daemon/provider audit/cost integration, hard budget enforcement, destructive recovery command, signing/publish/deploy automation은 후속 M9 slice로 남긴다.
