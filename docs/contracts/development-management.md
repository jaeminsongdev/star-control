...
설정 key와 병합 규칙은 [설정과 Catalog 계약](config-and-catalog.md), event와 materialized state 경계는 [이벤트와 상태 계약](events-and-state.md), 큰 출력과 gate 연결은 [검사·완료·증거](validation-and-evidence.md)가 소유한다.
...
| 계층 | 역할 | 정본 성격 | 대표 자료 |
|---|---|---|---|
| Git의 선언·Schema·Catalog·source | 팀이 검토·공유하는 의도와 구현 | 공유 정본 | `.star-control/project.toml`, config, Rule·ChangeRecipe 선언, suppression·baseline 선언, source code, contract type, generated Schema |
| 로컬 관리 DB | 검색·관계·현재 상태·이력 조회와 로컬 운영 상태 | source에서 다시 계산 가능한 projection 또는 명시적인 local-only state | Project 등록 projection, revision·snapshot, scan generation, symbol graph, Finding, local Disposition, active ChangePlan, event·idempotency index |
...
1. source code, Rule 정의, ChangeRecipe 정의와 공유 설정을 DB에서만 만들거나 DB 값으로 덮어쓰지 않는다.
...
|---|---|---|
| global store | Project directory projection, `root_binding_id`, identity scope, project-store locator, cross-project relation, `CoordinatedOperation`, global idempotency와 lifecycle summary | project source 절대 경로, source byte, project별 scan graph·Finding detail |
| project store | 해당 ProjectId의 revision·snapshot·source graph·scan·finding·local decision·change·validation·gate·ArtifactRef index | 다른 project의 detail, 다른 root binding, cross-project coordinator의 최종 상태 |
...
- `ManagementApplicationService`는 project 등록, scan, finding 조회·판정, change plan, patch, validation, gate, cross-store coordination과 store lifecycle use case를 소유한다.
...
| `project.refresh` | ProjectId, source precondition | ProjectRevision, WorkspaceSnapshot | 예 |
| `scan.run` | ProjectId, WorkspaceSnapshotId, config·rule fingerprint | ScanRun | 예 |
...
| `change.plan` | FindingId set, target snapshot, ChangeRecipe refs | ChangePlan | 예 |
| `patch.prepare` | ChangePlan revision, parameters | PatchSet | 예 |
| `patch.apply` | PatchSet fingerprint, workspace precondition, approval | 새 WorkspaceSnapshot | 예 |
...
Project는 하나의 관리 대상 source 경계다. Goal 안에서만 쓰는 기존 ProjectRef는 이 문서의 Project를 가리키는 작은 reference다.
...
ProjectRef의 persisted root reference는 raw LocalPathRef가 아니라 `root_binding_id`로 좁힌다. 실제 절대 root는 Controller process memory에서만 ProjectPathRef 해석에 사용한다.
...
Finding은 여러 scan에서 이어지는 논리적 문제 identity다. 원문 진단 instance인 Diagnostic과 달리 현재·과거 Occurrence를 묶는 materialized aggregate다.
...
| 필드 | 필수 | 규칙 |
|---|---:|---|
| `occurrence_id` | 예 | 실행 instance ID |
...
- subject의 ProjectRevisionId, WorkspaceSnapshotId와 content fingerprint
...
관리 Slice의 project 전용 입력은 `GateDecision.extensions["star.management"]`에
...
| ChangePlan | `draft -> ready -> applied -> validated`; 각 non-terminal에서 `blocked` 또는 `abandoned` | stale precondition은 `blocked`; 같은 revision을 다시 ready로 만들지 않고 plan revision 증가 |
...
| project/source | Project, ProjectRevision, WorkspaceSnapshot, CanonicalSource read·write |
| scan graph | ScanRun start, batch stage, generation finalize, Finding·Occurrence·Symbol·Reference query |
...
| project revision·snapshot·source·symbol graph 생성 | `star-project` |
...
| Patch execution lifecycle | `star-execution` |
