...
이 문서는 Star-Control 10단계인 **CI·Release·배포 준비, 규칙 평가와 최종 제품 완성**의 의미·상태·Gate·구현 순서 정본이다. P-0051의 `ReleaseManifest` v2·`EvaluationRun` v2·Catalog lifecycle·build-once engine과 P-0053 local release audit 위에 P-0054가 artifact byte 재검증, M3 evidence binding, promote/lifecycle, evaluation/catalog, exact release approval와 Controller·CLI를 연결했다. P-0055는 exact `ReleaseAssetBindingV1`과 `star-adapter-github`를 Controller publisher lifecycle에 연결했다. P-0056은 precommitted `EvaluationCaseDefinitionV1`·`EvaluationPolicyV1`, provider-verified `CostRecordV1`·derived `BudgetSnapshotV1`, actual Finding/Suppression metric, Radar 역참조와 역사적 `FinalProductAuditV1`을 추가했다. P-0058은 V1의 정적 owner/command 문자열만으로 생기던 false-ready를 폐쇄하고, generated `ProductSourceEvidenceV1`과 live `FinalProductAuditV2`가 owner 문서·Schema byte·owning handler·CLI/MCP/Codex 경로·4종 테스트·현재 source fingerprint를 함께 봉인하도록 교체했다. Authenticode signing과 public Stable publication은 실행하지 않았으며 unsigned Stable은 apply 전에 fail-closed다. 최신 독립 감사는 [P-0058 기능·Profile 전수 감사](../testing/p0058-feature-profile-independent-audit-2026-07-25.md)에 고정한다.
...
`star release audit <release-manifest-id>`는 위 표를 문서상 완료로 반복하지 않는다. build 시 `scripts/validation/check_product_inventory.py`가 exact 23개 inventory와 16개 Profile source를 읽고 owner 문서, manifest에 등록된 generated Schema와 hash, owning handler marker, CLI/Controller route, applicable MCP core/fixed action, Codex file marker, positive·negative·failure·recovery test를 현재 byte로 검증한 뒤 `ProductSourceEvidenceV1`을 만든다. live Controller는 이 evidence를 binary에 포함하고 내부 fingerprint·23/16 순서·4 Runtime EXE·generated Schema exact count·551 error·170 MCP matrix를 다시 검증한다.

그 뒤 실제 설치 우선 Profile Catalog가 exact 16 built-in set으로 resolve되는지, 각 version과 source evidence가 일치하는지, `rust_style_auto_fix`가 `refactor_codemod -> ai_development_validation` parent closure·format/lint/build/test·pre/post Gate·network denied policy를 유지하는지 계산한다. exact ReleaseManifest/artifact-set, source evidence, x64/ARM64 lifecycle record와 Profile Catalog fingerprint를 `FinalProductAuditV2`로 봉인한다. `FinalProductAuditV1`은 과거 record read 호환용이며 현재 완료 증거로 생성하지 않는다. 내부 source/profile conformance와 signing/provider 같은 외부 Gate는 별도 축이며 ARM64 Preview simulation은 `native_unverified` limitation이지 Stable native 성공이 아니다. Final audit는 support tier와 무관하게 포함 architecture의 `native_unverified`를 external gate reason으로 보존하므로 fake Preview evidence만으로 `conformant`가 되지 않는다. 반대로 실제 ARM64 registered adapter가 terminal effect receipt와 native lifecycle 결과를 제공하면 계약이 이를 영구 차단하지 않고 exact manifest compatibility와 receipt 검증 뒤 `native_verified`로 수용할 수 있다.
...
P-0055에서 14번의 비서명 부분은 구현·격리 검증했다. GitHub adapter는 local basename과 다른 `remote_name`을 `file#label`로 오해하지 않고 최대 512 MiB의 exact-name 임시 snapshot을 만든다. provider digest metadata가 있어도 모든 asset을 다시 download·hash하며, timeout 뒤 write 재시도는 금지한다. origin과 GitHub commit API가 exact candidate `b20d234`를 readback한 뒤 만든 disposable draft에서 local/provider/download SHA-256가 일치했고, release와 tag의 final absent cleanup까지 확인했다. draft는 publish하지 않았으므로 public release 증거로 승격하지 않는다.
