...
| M11 Rust style conformance | `rust_style_auto_fix` 의미 정본만 있고 제품 type·adapter·Corpus 없음 | P9 전에 stable toolchain·coverage·Patch/Gate·CLI-only x64/ARM64 evidence v6 요구 |
...
- clean Windows x64·ARM64 build·test·package·install lifecycle evidence
...
| `release` | 봉인된 artifact set | clean x64·ARM64 release environment와 설치 sandbox | package, file list, metadata, install/update/rollback/uninstall, safe_default, artifact digest·supply-chain·publish preflight | `ready`의 직접 Gate |
...
7. clean Windows environment spec, x64·ARM64 matrix, network/cache·dependency provisioning과 artifact retention을 고정한다.
...
필수 입력 하나라도 missing·stale·partial·unverified이면 build를 시작하지 않거나 `blocked` candidate로 종료한다. preflight 중 source·version·changelog·config를 자동 수정하지 않는다.

## clean Windows 지원 matrix

Star-Control 공개 runtime의 설계 target은 **Windows 11 24H2 build 26100 이상, x64·ARM64**다. 이 baseline은 제품 지원 의도이며 실제 release 지원 주장은 다음 evidence가 있을 때만 가능하다.

| target | build evidence | runtime evidence | install lifecycle evidence |
|---|---|---|---|
| `x86_64-pc-windows-msvc` | clean native 또는 approved clean builder의 exact binary·toolchain | native x64 process·IPC·Controller·CLI·MCP | clean install·safe_default·update·rollback·uninstall |
| `aarch64-pc-windows-msvc` | clean ARM64 builder 또는 cross-build provenance | **native ARM64** process·IPC·Controller·CLI·MCP; cross-compile만으로 대체 불가 | native ARM64 clean install·safe_default·update·rollback·uninstall |
...
CI provider에 native ARM64 runner가 없으면 release policy를 x64-only로 조용히 낮추지 않는다. ARM64 target evidence limitation은 `blocked_external`, ReleaseManifest 전체 상태는 `blocked`로 기록하며 `external_gates`에 원인을 연결한다. 지원 matrix·문서·package list에서 완료로 표시하지 않는다. 지원 baseline 변경은 release 한 번의 우회가 아니라 versioned policy·문서·compatibility change다.
...
- public binary·installer의 실제 요구는 P9 구현 직전 현재 Windows·배포 채널·signing service 공식 문서를 다시 조사한다.
...
| `release_verify` | final artifact digest·full checks | x64·ARM64 required run과 artifact subject 일치 |
  ...
  -> ready                                |
       -> approved                        |
            -> publishing                 |
                 ...
                 -> rollback_required     |
published -> withdrawn                    |
blocked/publish_outcome_unknown/rollback_required -> 새 preflight·approval·operation revision
```
...
clean install evidence는 다음 순서를 실제 disposable Windows environment에서 확인해야 한다.

1. 기존 Star-Control program file·Controller startup entry·user config가 없는지 확인한다.
2. installer가 expected x64 또는 ARM64 file set만 설치한다.
...
| 구분 | 현재 상태 |
|---|---|
| 구현됨 | MCP 기반 수직 Slice, P0 공통 개발 관리 첫 수직 Slice, P-0026 Windows installer·installation record·Codex Plugin 렌더링 transport Slice |
| 설계 확정·구현 전 | M1~M11 전체와 이 10단계 release/evaluation, 최종 16 Profile 완성, release/evaluation engine |
| 외부/환경 gate | current 통합 binary의 Codex·Inspector evidence, required core 13개 owner, current native ARM64와 exact 24H2·clean x64/ARM64 수명주기, signing 필요성·credential, CI/registry/deploy provider |
...
14. 실제 clean Windows x64·native ARM64·installer·CI·signing·registry/deploy adapter를 하나씩 연결
...
- clean x64·ARM64 install과 safe_default first-run
...
- clean Windows x64·ARM64 build·test·package·native runtime·install evidence 요구가 명확하다.
...
- `rust_style_auto_fix`가 stable pinned toolchain·exact allowlist·complete coverage·isolated PatchSet·idempotence·`personal_auto` exact approval와 Windows x64/ARM64 CLI-only evidence를 가진다.
...
- required core 13개 owning handler·Schema와 current MCP completion evidence
- historical success가 아닌 current 통합 binary의 Codex·Inspector integration evidence
- current native Windows ARM64 runner/device, exact Windows 11 24H2 baseline과 clean x64/ARM64 install lifecycle 환경
- clean disposable x64·native ARM64에서 current-user install·update·rollback·uninstall의 실제 수명주기 evidence
- public channel별 SBOM·provenance·code signing 필요성, signer·credential·비용 승인
- CI/artifact registry/release/deploy provider별 immutable artifact·digest·after-state adapter conformance
- M11 Rust ToolDescriptor/Catalog/Schema, complete Corpus와 native Windows x64·ARM64 CLI-only conformance
- 실제 publish·deploy·withdrawal·remote/account effect의 현재 사용자 승인
