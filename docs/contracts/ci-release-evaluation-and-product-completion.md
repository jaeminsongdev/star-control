...
`star release audit <release-manifest-id>`는 위 표를 문서상 완료로 반복하지 않는다. build 시 `scripts/validation/check_product_inventory.py`가 exact 23개 inventory와 16개 Profile source를 읽고 owner 문서, manifest에 등록된 generated Schema와 hash, owning handler marker, CLI/Controller route, applicable MCP core/fixed action, Codex file marker, positive·negative·failure·recovery test를 현재 byte로 검증한 뒤 `ProductSourceEvidenceV1`을 만든다. live Controller는 이 evidence를 binary에 포함하고 내부 fingerprint·23/16 순서·4 Runtime EXE·generated Schema exact count·528 error·170 MCP matrix를 다시 검증한다.
