...
P-0055에서 14번의 비서명 부분은 구현·격리 검증했다. GitHub adapter는 local basename과 다른 `remote_name`을 `file#label`로 오해하지 않고 최대 512 MiB의 exact-name 임시 snapshot을 만든다. provider digest metadata가 있어도 모든 asset을 다시 download·hash하며, timeout 뒤 write 재시도는 금지한다. 다만 host가 commit을 막아 authenticated draft의 target은 staged candidate commit이 아니라 base commit이었으므로 exact candidate publication evidence로 승격하지 않는다.
...
P-0055 candidate index tree `2eb3680b3f0cf5a8ae6b0daadff6fe54f003e067`은 final FULL 10/10, x64/ARM64 473-file stage·PE architecture·installer model, x64 isolated finalize/Bridge/status, ARM64 fake lifecycle, SPDX SBOM·RustSec audit·pre-sign provenance와 official Inspector fixed 12/12·core search/describe 17/17을 통과했다. authenticated disposable draft asset은 provider/download SHA-256가 일치했고 release/tag cleanup도 확인했다. 그러나 host policy가 commit을 막아 release clean-worktree가 fail이고 current installed Codex는 registry revision 4의 ready core 6개를 유지한다. candidate Inspector run을 current Codex 설치 증거로 승격하지 않는다.
...
- host가 허용하는 경로로 staged tree를 commit·push하고 exact candidate commit target의 draft/readback을 다시 만든다.
...
P-0040~P-0053 historical 구현은 clean revision chain으로 보존된다. P-0054/P-0055 current source는 staged tree까지 구현·검증됐지만 clean commit이 아니므로 release 상태는 `ready`가 아니다. usable Authenticode certificate·timestamp provider와 signed candidate·current Codex integration이 없으므로 public release 상태도 `blocked_external`이다. local 구현 완료, disposable draft, unsigned installer model 또는 ARM64 simulation을 `ready|approved|published`로 승격하지 않는다.
