...
P-0053 local audit은 required core 17/17 source readiness, 현재 설치본이 제공하는 6개 action의 search/describe/invoke, current Inspector `tools/list`, x64 release build·격리 lifecycle model과 ARM64 cross-build·PE architecture·file manifest·installer model·fake lifecycle을 재생성했다. M11 Rust ToolDescriptor/Catalog/Schema와 bounded Corpus도 current FULL에 포함된다. 다만 현재 설치본은 source candidate가 아니므로 새 11개 action의 installed-runtime 실행 증거로 승격하지 않으며, active Controller를 중단하지 않고 candidate Inspector를 연결할 수 없어 그 항목은 외부 설치 lifecycle에 남긴다.
...
- SBOM·provenance와 exact GitHub repository/tag/channel/asset digest approval을 결합한다.
...
현재 P-0040~P-0053 변경은 아직 local commit으로 봉인되지 않아 immutable candidate Gate가 먼저 `BLOCK`이다. 이를 clean source revision으로 봉인한 뒤에도 usable Authenticode certificate·timestamp provider와 remote effect 승인이 없으므로 public release 상태는 `blocked_external`이다. local 구현 완료를 release 완료로 해석하거나 unsigned Stable, source candidate 미설치, ARM64 simulation을 `ready|approved|published`로 승격하지 않는다.
