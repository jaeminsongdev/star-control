...
0단계 공통 개발 관리 의미는 [공통 개발 관리와 로컬 관리 DB 계약](development-management.md), [ADR-0006](../decisions/ADR-0006-공통-개발-관리와-로컬-관리-DB-경계.md)과 [ADR-0007](../decisions/ADR-0007-P0-하이브리드-저장소와-운영-정책.md)에 확정했다. P0 inventory의 persisted type은 `star-contracts` type, generated JSON Schema, minimal/full/invalid/future fixture와 fingerprint golden으로 구현했다. P-0041~P-0043은 `ProjectCheckout`, `ProjectCatalogSnapshot`, `CodeIndexSnapshot`과 full planning bundle(`TaskSpec`, `ScopeRevision`, `ChangeSet`, `ImpactAnalysis`, `RiskPathDescriptor`, `ValidationPlan` v2)을 추가했다. P-0044~P-0052는 M3 CheckGraph/evidence와 Goal/Plan/Run, M4 immutable PatchSet, M5~M9 development engine·당시 17/17 core source readiness, M10 ReleaseManifest/EvaluationRun build-once engine, M11 Rust fixed pipeline의 첫 bounded 제품 Slice를 구현했다. P-0075는 기존 application command를 재사용해 project register·scan·index·Finding·Diagnostic 6개 action을 additive로 연결하며 current required core를 23/23으로 확장한다. P-0030/P-0031의 exact-root status와 `tracked_path_precursor`는 별도 v1 운영 precursor로 유지한다.

P-0054는 최신 `main`에서 Recovery Slice, M1~M11 내부 제품 경로와 최종 16 Profile Catalog를 contract→engine→repository→Controller→CLI로 확장했다. P-0056은 current evidence·effect receipt, evaluation source policy/case, verified cost/budget, actual Finding/Suppression metric과 23/16 final audit를 추가했다. P-0062는 [Code Health·장기 유지보수 계약](code-health-and-maintainability.md)으로 SARIF와 source/Git-derived 관측의 기존 contract 재사용 경계를 설계했고 P-0063~P-0070은 Schema·engine·repository·Controller·CLI 경로를 구현했다. P-0075는 이 경로의 register·scan·index·Finding·Diagnostic 진입점을 required core MCP action으로 연결한다. 현재 구현과 검증 범위는 `PLANS.md`, `docs/testing/m1-*`~`m11-*`, [P-0054 실제 기능 완성 감사](../testing/p0054-functional-completion-audit-2026-07-23.md)와 [P-0056 최신 감사](../testing/p0056-current-functional-recovery-audit-2026-07-24.md)가 소유한다. 외부 scanner·debugger·package-manager mutation, live migration/cutover, authenticated remote, signer·publisher와 ARM64 native 실행은 내부 구현 완료와 분리된 Gate이며 실행하지 않은 효과를 완료로 표시하지 않는다. P-0056 exact source의 x64 격리 lifecycle, ARM64 `native_unverified` simulation과 authenticated GitHub unpublished draft byte 왕복·cleanup은 완료했지만, Authenticode·trusted timestamp와 signed clean lifecycle·public Stable publish/readback은 없어 계속 `blocked_external`이다. MCP exact 구현 값은 [MCP 구현 동결 계약](mcp-implementation-contract.md), [ToolPackageManifest Reference](tool-package-manifest-reference.md), [Windows Tool Runtime](../architecture/windows-tool-runtime.md)과 [MCP 검증 행렬](../testing/mcp-verification-matrix.md)에 동결됐고, current source와 설치본의 차이는 [MCP 완료 감사](../testing/mcp-completion-audit.md)에 분리한다.
...
| RustStylePolicyApprovalDecision | `star.rust-style-policy-approval-decision` | [Rust style](../features/rust-code-style-auto-fix.md) | durable human/policy-evaluator 결정 |
...
| DevelopmentProfileDescriptor | `star.development-profile-descriptor` | [설정·Catalog](config-and-catalog.md) | 정확한 built-in 16 source descriptor |
| DevelopmentProfileCatalogSnapshot | `star.development-profile-catalog-snapshot` | [설정·Catalog](config-and-catalog.md) | strict load·definition hash·parent 검증 결과 |
| DevelopmentProfileResolution | `star.development-profile-resolution` | [설정·Catalog](config-and-catalog.md) | selected ID·closure·merged floor·fingerprint |
| PolicyProfileDescriptor | `star.policy-profile-descriptor` | [설정·Catalog](config-and-catalog.md) | Catalog·permission 설정 |

`crates/foundation/star-contracts`는 v1 precursor와 P-0054의 TaskSpec·ScopeRevision·ImpactAnalysis, exact subject binding, completeness·freshness·stability, Diagnostic/Rule/Baseline/Suppression/Disposition v2, Patch 전·후 Gate, Recovery와 M5~M11 공개 계약을 함께 소유한다. `TaskSpec.profile_ids`, `ValidationPlan.profile_resolution`과 Evidence의 `profile_resolution_fingerprint`가 동일 resolution을 연결하며 M3 runner는 M2가 선택한 required Check floor를 약화하지 않는다. 외부 adapter도 이 crate의 type과 checked-in Schema만 소비하며 동등한 완료·진단 DTO를 별도로 정의하지 않는다.
