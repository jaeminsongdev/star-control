...
P1 구현은 이 문서의 inventory와 각 상세 계약을 `star-contracts` type, JSON Schema와 fixture로 옮기는 작업이다. MCP 부분의 exact 구현 값은 [MCP 구현 동결 계약](mcp-implementation-contract.md), [ToolPackageManifest Reference](tool-package-manifest-reference.md), [Windows Tool Runtime](../architecture/windows-tool-runtime.md)과 [MCP 검증 행렬](../testing/mcp-verification-matrix.md)에 동결됐다. Schema 파일과 제품 코드는 아직 만들지 않았다.
...
4. 상태 snapshot은 빠른 조회용이고 EventEnvelope가 변경 이력의 근거다.
5. 원문 log와 큰 결과는 계약 안에 넣지 않고 hash가 있는 ArtifactRef로 연결한다.
6. secret, token, 개인정보 원문은 어떤 계약에도 허용하지 않는다.
...
ID는 대소문자를 구분하는 ASCII string이며 재사용하지 않는다. 사람이 붙이는 제목과 branch 이름은 ID가 아니다.
...
| `ActorRef` | `actor_type`, `actor_id`, `display_name`, `auth_source` | user 의도를 전달한 actor와 user 자체를 구분 |
| `ProjectPathRef` | `project_id`, slash 기반 상대 `path`, `path_kind` | project root 이탈 금지 |
| `LocalPathRef` | local anchor, 암호화·가림 가능한 path | Controller 내부에서만 원문 허용 |
...
현재 Inventory는 49개 계약 항목이다. `IpcRequest·Response`와 고정 MCP surface처럼 한 행이 여러 generated Schema를 소유할 수 있으므로 이 수는 실제 `.schema.json` 파일 수와 같다는 뜻이 아니다.
...
| EventEnvelope | `star.event` | [이벤트·상태](events-and-state.md) | `events.jsonl` |
| RunSnapshot | `star.run-snapshot` | [이벤트·상태](events-and-state.md) | Controller state |
...
| A03 프로젝트 이해·Context | ProjectRef, ContextPack, SourceRecord |
| A04 변경 영향·위험 분석 | ChangeSet, Diagnostic, ValidationPlan |
...
| B01 변경·범위·주장·증거 | ChangeSet, EvidenceBundle, ReviewPack |
| B02 테스트 신뢰성 | CheckDescriptor, ValidationRun, Diagnostic |
| B03 검증기 보호·Corpus | Diagnostic, GateDecision, EvaluationRun |
| B04 계약·구조·설정·migration | ValidationPlan, EffectiveConfig, version 계약 |
