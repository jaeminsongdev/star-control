...
| `stage_id` | 대상 Stage |
| `base_revision` | 자료 선택 기준 revision |
| `items` | ContextItem 목록 |
| `token_budget` | 전달 가능한 대략적 한도 |
| `omissions` | 제외한 자료와 이유 |
| `generated_at` | 생성 시각 |

ContextItem은 `kind`, ProjectId, 상대 경로 또는 URI, revision·hash, 포함 이유, source authority, freshness, sensitivity, 전달 방식과 limitation을 가진다. `sensitivity=secret`인 항목은 원문 전달을 금지하고 존재와 필요한 권한만 표시한다.
