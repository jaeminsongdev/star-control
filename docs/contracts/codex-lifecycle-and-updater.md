...
`star-updater.exe`가 바뀐 후보와 runtime/integration 혼합 후보는 integration-only
apply 대상이 아니며 offline installer 또는 분리된 candidate가 필요하다. full/mixed
offline installer는 setup 뒤 새 release manifest가 소유한 bundled Runtime Generation을
별도로 reconcile하고 live release Registry exact-set postcheck까지 통과해야 한다.
...
replacement setup의 `installation bridge initialize`는 existing activation selector를
임의 승격하지 않는다. setup 성공 뒤에도 detached Updater는 verified root manifest가
소유한 정확히 한 generation을 선택하고 prior selector를 보존한 채 activation한다.
새 Controller의 release action 전체 집합과 `ready` 집합이 candidate package manifest의
ToolId 집합과 모두 정확히 같아야 `offline_verified`다. stale selector 유지, 복수
manifest-owned generation, 누락·non-ready action은 실패다. candidate activation 뒤
postcheck가 실패하면 prior selector와 Controller를 복원하되 replacement fixed files가
...
| `partially_applied` | replacement fixed files는 남고 prior Runtime selector만 복원됨; 완료·full rollback 아님 |
...
`partially_applied`, `rollback_failed`는 각각 installation 결과를 말할 뿐 Codex relaunch를 생략한다는
