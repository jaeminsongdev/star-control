...
freshness는 Project Catalog, source inventory, text, syntax, semantic, graph와 Finding partition별로 계산한다. text가 current여도 semantic partition만 stale일 수 있다.
...
2단계는 다음 입력을 exact reference로 받는다.
