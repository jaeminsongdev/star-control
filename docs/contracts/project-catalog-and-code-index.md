...
- `star.core.project.register`와 `star.core.scan.run`만 `write_closed`이며 exact current project·ProjectId와 idempotency key에 결속한다. `star.core.index.status`, `star.core.index.search`, `star.core.finding.list`, `star.core.diagnostic.list`는 `read_closed`이고 stale/current·coverage·limitation을 숨기지 않는다.
