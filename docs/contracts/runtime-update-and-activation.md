...
현재 P-0038 구현은 persisted contract/schema, Bootstrap의 generation selector, Windows activation record 원자 writer, verified generation stage, candidate review, stable CLI supervisor와 rollback까지 포함한다. Registry action은 MCP의 고정 진입점을 넓히지 않기 위해 별도 release package로만 후속 도입하며, installed-tree same-task E2E는 설치 상태를 바꾸므로 명시적 실행 승인이 필요하다.
...
`star update apply <generation-id> --state-generation <id> --approve <sha256>`는 bridge compatible, handler ready, rollback available, candidate verification pass, approval scope 일치를 모두 요구한다. action 제거, breaking schema, risk lane/permission widening 또는 hook/new-task/Codex restart가 필요한 review는 apply하지 않는다. Stable root `star.exe`가 인증된 Controller shutdown을 요청하고, quiesce를 bounded wait로 확인한 뒤 selector를 원자 교체하고 새 Runtime Controller를 시작·IPC postcheck한다. 새 Controller 시작 또는 postcheck 실패 시 prior generation을 다시 selector로 만들고 재기동한다. Codex와 고정 MCP는 이 동안 재시작하지 않는다.
...
Controller가 종료된 뒤에도 Stable root CLI supervisor가 activation과 새 Controller start를 담당한다. Runtime Controller는 launch argument로 Bootstrap install root를 받으며, IPC peer 검증은 generation-local CLI와 fixed Bootstrap의 `star.exe`/`star-mcp.exe`만 허용한다. MCP는 cutover 중 mutation을 재시도하지 않고 `CONTROLLER_UPDATING` 또는 명시적 bounded wait를 반환한다.
...
3. apply/rollback은 exact approval scope에 바인딩되며 stable CLI supervisor가 Controller shutdown, selector 교체, 새 Controller postcheck와 rollback을 소유한다.
