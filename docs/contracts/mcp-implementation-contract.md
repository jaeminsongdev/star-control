...
cursor는 base64url-no-padding JCS object `{snapshot_hash,query_hash,last_score,last_tool_id}`다. `query_hash`는 cursor를 제외한 normalized query·filter·limit object의 JCS SHA-256이다. 현재 snapshot hash 또는 query hash가 다르면 `TOOL_SEARCH_CURSOR_STALE`이다.
...
같은 decision·scope의 반복은 기존 결과를 반환한다. 다른 decision 또는 stale scope는 `POLICY_APPROVAL_STALE`이다. 이 tool은 승인된 action을 직접 실행하지 않고 대기 Operation을 다시 runnable로 만든다.
