...
Controller pipe가 readiness 안에 나타나지 않으면 `IPC_CONTROLLER_UNAVAILABLE`, HMAC 실패는 `IPC_AUTH_FAILED`, PID·설치 image 불일치는 `IPC_SERVER_IDENTITY_MISMATCH`, IPC major 불일치는 `IPC_PROTOCOL_MISMATCH`로 정규화한다. restrictive outer Job의 fixed MCP는 검증된 Controller command만 local WMI broker로 기동하고 반환 PID image를 lease와 다시 대조한 뒤 같은 bounded readiness를 사용한다. 수동 CLI bootstrap은 MCP 가용성의 선행 조건이 아니다. 인증을 통과한 뒤 malformed·truncated·oversized response frame은 인증 실패로 섞지 않고 `IPC_FRAME_INVALID`로 반환한다.
...
| `star.core.project.register` | `project.register` | write_closed |
| `star.core.scan.run` | `scan.run` | write_closed |
| `star.core.index.status` | `index.status` | read_closed |
| `star.core.index.search` | `index.search` | read_closed |
| `star.core.finding.list` | `finding.list` | read_closed |
| `star.core.diagnostic.list` | `diagnostic.list` | read_closed |
| `star.core.validation.plan` | `validation.plan` | read_closed |
| `star.core.validation.run` | `validation.run` | write_closed |

core action의 input·output 의미는 각 application command의 소유 계약을 그대로 사용한다. MCP package가 필드를 새로 만들거나 축약하지 않는다. build 단계에서 소유 계약의 generated Schema 또는 package-owned local Schema를 가져와 remote `$ref` 없는 fully resolved inline Schema로 만들고, command가 없거나 Schema version mapping이 없으면 release build를 실패시킨다. 이 23개 action의 ToolId·command·lane만 이 문서가 소유한다. action별 readiness는 package/manifest readiness와 분리하며, concrete handler와 input·output Schema가 모두 연결된 action만 `ready`다.

## Codex MCP 설정 정본

Installer가 렌더링한 Plugin `.mcp.json`의 의미:

```json
{
  "mcpServers": {
    "star-control": {
      "command": "<actual-install-root>\\star-mcp.exe",
      "args": []
    }
  }
}
...
`<actual-install-root>`는 literal이나 기본 경로가 아니다. `star integration install|repair`가 검증된 installation record의 실제 절대 경로로 바꾼 뒤 Codex 공식 Plugin 경로로 등록한다. 정적 template, package source와 MCP binary에는 특정 PC의 설치 경로를 넣지 않는다. 자세한 소유권과 등록 절차는 [Windows 설치와 Codex 연동 계약](windows-installation-and-codex-integration.md)이 소유한다.

Plugin 밖에서 동등한 server를 수동 등록할 때의 approval 기준은 다음과 같다. Installer는 Codex `config.toml`이나 Hook trust 저장소를 직접 수정하지 않는다.
