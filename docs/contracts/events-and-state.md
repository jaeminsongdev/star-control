...
Star-Control은 현재 상태만 저장하지 않는다. 무엇이 어떤 순서와 이유로 바뀌었는지 EventEnvelope로 남기고, 빠른 조회를 위한 RunSnapshot을 그 이벤트에서 만든다. Controller만 상태를 쓰며 CLI, MCP와 child Codex는 application command를 보낼 뿐 직접 파일을 수정하지 않는다.

저장 위치와 보관 원칙은 [상태 기록과 이어하기](../architecture/state-and-artifacts.md), 공통 ID·시간·경로 형식은 [데이터 계약 지도](README.md)가 소유한다.
...
| `sequence` | positive integer | Run 안에서 빈틈 없이 증가 |
...
| `previous_event_hash` | optional SHA-256 | 앞 event hash. 첫 event는 생략 |
| `payload` | typed object | event별 최소 변경 자료 |
| `artifact_refs` | ArtifactRef array | 큰 입력·출력 |
| `event_hash` | SHA-256 | canonical event 내용의 hash |

ActorRef는 `actor_type`, stable ID, 표시 이름과 인증 출처를 가진다. Codex가 대화에서 사용자의 말을 전달한 경우 actor는 user로 위조하지 않고 `actor_type=codex`, `asserted_user_intent=true`와 근거 correlation을 남긴다.
...
Snapshot은 직접 patch하지 않는다. command handler가 새 event를 commit한 뒤 reducer가 새 snapshot을 만들며, 불일치하면 event log에서 재구축한다.
...
1. Controller 한 process만 event와 snapshot을 쓴다. current-user 단일 instance lock을 얻지 못하면 두 번째 Controller는 시작하지 않는다.
2. 모든 mutating command는 `expected_revision`을 받는다. 현재 revision이 다르면 `STATE_REVISION_CONFLICT`로 거부하고 최신 snapshot을 돌려준다.
3. 재시도 가능한 command는 `idempotency_key`가 필수다. 같은 key와 같은 payload는 이전 결과를 돌려주고, payload가 다르면 충돌이다.
4. 큰 artifact는 먼저 임시 위치에 쓰고 hash를 검증한 뒤 안전하게 교체한다. 이후 이를 참조하는 event를 append하고 durable flush한다.
5. event commit 뒤 snapshot을 임시 파일에 만든 다음 같은 volume에서 atomic replace한다. snapshot 교체 전에 중단돼도 event replay로 복구한다.
6. 마지막 JSONL 행이 잘렸거나 hash chain이 맞지 않으면 조용히 버리지 않고 격리, Diagnostic과 recovery event를 만든다.
