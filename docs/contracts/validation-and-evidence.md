...
`ProductSourceEvidenceV1`은 A01~D03 exact 23개 feature마다 owner 문서, generated Schema hash, owning handler marker, CLI/Controller command, applicable MCP action과 Codex source ref, positive·negative·failure·recovery test를 current source byte에 묶는다. 같은 record가 Profile 16개의 raw definition hash·version, activation-input fingerprint, required rule/check·Gate·permission·approval·effect·unknown-outcome·rollback policy fingerprint와 공통 resolution/validation/permission/recovery/rollback handler·test 10개, Runtime EXE 4개, generated Schema count, stable error 552개와 MCP matrix 172개를 봉인한다. build validation이 새 source와 evidence 불일치를 허용하지 않으며 runtime은 embedded record의 내부 fingerprint와 exact inventory를 다시 검증한다.
