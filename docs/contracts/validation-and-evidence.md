...
Star-Control 3단계의 실행 순서·rule family·ratchet·validator guard·Patch 적용 전후 의미는 [공통 검증·품질 Gate 상세 설계](../features/common-validation-gate.md)가 소유한다. 4단계의 Recipe selector·rewrite assurance·dry-run·idempotence·apply·복구 알고리즘은 [안전한 Patch·Refactor·codemod 엔진 계약](safe-patch-and-codemod.md), 5단계 identity·lifecycle·binding·consumer는 [Managed Registry](managed-symbol-registry.md), 6단계 compatibility·docs/config/environment 판정은 [계약 호환성·환경](contract-compatibility-and-environment.md), 7단계 failure·supply-chain·dependency·Radar 의미는 [실패 재현·보안·의존성 유지보수](failure-security-and-dependency-maintenance.md), 8단계 migration·performance·language/platform 의미는 [Migration·성능·언어·플랫폼 계약](migration-performance-and-platform.md), 9단계 project/worktree/merge/remote coordination은 [CrossRepo ChangeBundle](cross-repo-change-bundle.md), 10단계 release/evaluation 상태·Gate·판정은 [CI·Release·평가 정본](ci-release-evaluation-and-product-completion.md), 11단계 Rust style pipeline·coverage·policy 의미는 [Rust 코드 스타일 자동 교정](../features/rust-code-style-auto-fix.md)이 소유한다. 이 문서는 M3~11단계가 사용하는 evidence wire 계약의 유일한 정본이다. P-0044는 아래 계약 중 공통 `TaskInvocation`·`ValidationRun`·`Diagnostic`·`GateDecision`·`EvidenceBundle` v2 수직 Slice와 CheckGraph runner를 구현했다. 뒤 단계 전용 확장은 해당 단계에서 구현되기 전까지 목표 계약이다.

## P-0044 구현 경계

- `star-contracts::evidence_v2`가 같은 schema identity의 v2 wire type과 fingerprint·false-pass invariant를 소유하고 generated Schema·minimal/full/invalid/future fixture를 제공한다.
- `star-validation::runner`는 ready ValidationPlan의 선언된 DAG와 executable binding만 소비한다. cycle은 실행 전 거부하고 dependency 실패는 `not_run`, timeout·partial·flaky·unverified는 blocking Diagnostic과 `BLOCK`으로 보존한다.
- `ManagementApplicationService::execute_planning_bundle`은 M1/M2 source snapshot을 실행 전후 재확인하고 ValidationRun·Diagnostic·GateDecision·EvidenceBundle을 Project SQLite transaction 하나로 저장한다.
- protected Goal store와 core handler는 `goal.start/answer/status/pause/resume/cancel`, `plan.get/update`, `run.continue`를 revision·idempotency 경계로 제공한다. P-0044 종료 시 required core는 15/17이며 `merge.status`, `handoff.get`은 M9 전까지 unavailable이다.
- M7~M11 전용 Rule family, ReviewPack·migration·release·Rust-style 확장은 이 Slice가 구현했다고 보지 않는다.
...
아래 `task_spec_ref`부터 `readiness`까지의 확장은 P-0031 당시 2단계 **목표 계약**이었다. 현재 P-0043은 `TaskSpec → ScopeRevision → ChangeSet → ImpactAnalysis → ready ValidationPlan` v2 수직 Slice와 deterministic fingerprint를 구현했으며 계산 규칙은 [변경 계획·영향 분석 정본](change-planning-and-impact.md)이 소유한다.

P-0031의 같은 schema ID v1 `capability_level=tracked_path_precursor`는 historical input으로 남는다. P-0043 v2 bounded contract는 changed file/source/class, direct unit, reverse consumer, adaptive profile과 이유, exact planned command, cache key 구성요소·execute/reuse 판정, uncertainty, independent-review trigger와 canonical evidence flow를 담는다. cache reuse와 AI 압축은 ValidationRun·GateDecision·EvidenceBundle·Diagnostic의 immutable ID·revision/sequence·canonical hash를 대조하며, 자유 문자열로 명령·종료 코드·소요시간을 바꾸지 않고 ValidationRun에서 파생한다.

evidence 순서는 `ValidationPlan -> ValidationRun/Diagnostic/ValidationResult -> GateDecision -> EvidenceBundle -> AI compressed summary`로 고정한다. AI summary는 EvidenceBundle과 GateDecision의 exact ref가 검증된 뒤 명령, 종료 코드, 소요시간, 실패 요약과 남은 위험 수만 노출하며 raw log나 artifact 내용을 재판정하지 않는다. P-0035의 bounded `validation.run`은 exact tracked `scripts/validate.ps1`을 실행해 sealed plan과 native report를 대조하고, `evidence.get`은 그 immutable report path·hash를 재검증한다. 동일 입력의 complete·stable pass만 프로젝트별 derived cache에서 재사용한다. cache hit 직전과 성공 결과 저장 직전에 같은 check ID·cache key를 다시 관찰하며, 입력 drift, report binding 불일치 또는 artifact hash 불일치가 있으면 cache를 사용하지 않고 native 실행 또는 미저장으로 되돌린다. P-0044의 CheckGraph runner와 Controller transaction writer는 Diagnostic·ValidationResult normalization, authoritative GateDecision과 EvidenceBundle v2를 current source에 구현했다.
...
`ProductSourceEvidenceV1`은 A01~D03 exact 23개 feature마다 owner 문서, generated Schema hash, owning handler marker, CLI/Controller command, applicable MCP action과 Codex source ref, positive·negative·failure·recovery test를 current source byte에 묶는다. 같은 record가 Profile 16개의 raw definition hash·version, activation-input fingerprint, required rule/check·Gate·permission·approval·effect·unknown-outcome·rollback policy fingerprint와 공통 resolution/validation/permission/recovery/rollback handler·test 10개, Runtime EXE 4개, generated Schema count, stable error 528개와 MCP matrix 170개를 봉인한다. build validation이 새 source와 evidence 불일치를 허용하지 않으며 runtime은 embedded record의 내부 fingerprint와 exact inventory를 다시 검증한다.

`FinalProductAuditV2`는 exact ReleaseManifest/artifact-set, 위 source evidence, current Profile Catalog fingerprint와 exact 16 runtime resolution, M11 closure, architecture별 receipt-bound lifecycle evidence를 봉인한다. Profile별 runtime status는 raw source definition, activation/policy fingerprint, current Project Catalog·WorkspaceSnapshot·CodeIndex, effective config, 발견된 toolchain, parent closure의 required Check 전체 coverage와 승인·unknown outcome·rollback 실행 경로를 함께 요구한다. 하나라도 불일치하면 `profile_runtime_binding_incomplete` limitation으로 downgrade하며 caller가 넣은 `conformant=true`를 신뢰하지 않는다. `internal_conformance`와 `status=conformant|blocked_external|blocked`를 분리한다. `FinalProductAuditV1`은 과거 호환 record일 뿐 current release 완료 증거가 아니다. ARM64 Preview fake/cfg/target simulation은 `native_unverified`로 허용하지만 ARM64 Stable native pass로 승격할 수 없고, missing lifecycle·Stable non-native·required signing/provider는 external Gate로 남는다.
