...
`profile.list|show|resolve`는 설치 catalog만 읽는 Controller-owned read path이며 management DB를 요구하지 않는다. 따라서 management runtime이 `Initializing`, recovery-only 또는 retention maintenance 중이어도 인증·payload validation 뒤 현재 catalog에서 응답한다. 이 가용성은 resolve 결과 자체를 실행 완료나 Profile conformance 증거로 승격하지 않는다.
...
| `required_check_families` | affected target, clean full build/test/docs/contract/security, package dry-run, x64 Stable runtime/install, ARM64 Preview cross-build/simulation, update/rollback/repair/uninstall, remote verify |
...
| `platform_policy` | x64 signed Stable native runtime/install과 ARM64 signed `native_unverified` Preview cross-build/simulation; Preview를 native pass로 승격 금지 |
...
3. clean x64 Stable environment와 ARM64 Preview cross-build·simulation environment, locked dependency·network/cache policy를 고정한다.
...
7. native x64 clean install, `safe_default` first run, update, failure rollback, repair와 uninstall data preservation을 검사하고 ARM64는 model-equivalent fake lifecycle을 별도 검사한다.
...
10. publish·deploy adapter receipt 뒤 remote after snapshot이 exact version·source·artifact digest를 확인할 때만 `published`로 전이한다. Star-Control `v0.1.0` policy는 GitHub Release publication만 사용하고 deploy action을 만들지 않는다.
